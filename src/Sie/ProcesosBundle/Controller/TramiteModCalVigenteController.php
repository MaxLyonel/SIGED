 ...
 *
 */
class TramiteModCalVigenteController extends Controller {
    public $session;
    ...
    public function indexAction (Request $request) {
        ...
        return $this->render('SieProcesosBundle:TramiteModCalVigente:index.html.twig', array(
            'flujoTipo'=>$flujoTipo,
            'historial'=>$historial,
            'idTramite'=>$idTramite,
            'sieActual'=>$ie_id
...
// dump($datos);die;
    ...
    public function regularDBmgv($idInscripcion,$operativo){
        // dump('aqui new'); die;
            ...
            foreach ($asignaturas as $a) {
                ...
                if ($nivel != 11)  {
                    ...
                    for($i=$inicio;$i<=$fin;$i++){
                        ...
                        if($em->getRepository('SieAppWebBundle:RegistroConsolidacion')->findOneBy(array('unidadEducativa'=>$sie, 'gestion'=>$gestion, "$arrConsolidation[$i]"=> 2))){
                        ...
                        foreach ($asignaturasNotas as $an) {
                            ...
                            if($i == $an['idNotaTipo']){
                                ...
                                // }
                                $swCenso = 0;
                                $nota = $em->getRepository('SieAppWebBundle:EstudianteNota')->findOneBy([
                                    'estudianteAsignatura' => $an['idEstudianteAsignatura'],
                                    'notaTipo' => [59, 60] // Buscar en ambos tipos de nota
                                ]);
                                
                                if (($nota && $nota->getNotaTipo()->getId() == 59 && $i == 7) || ($nota && $nota->getNotaTipo()->getId() == 60 && $i == 8)) {
                                    $swCenso = 1;
                                }
    ...
    public function formularioVistaImprimirLibretaAction(Request $request){
        try {
            ...
            return $this->render('SieProcesosBundle:TramiteModCalVigente:formularioVistaImprimirLibreta.html.twig', array(
                'historial'=>$historial,
                'datos'=>$datosNotas,
                'idTramite'=>$idTramite,
                'idInscripcion' => $idInscripcion,
    ...
    public function recepcionVerificaDistritoAction(Request $request){
        ...
        return $this->render('SieProcesosBundle:TramiteModCalVigente:formularioVistaDistrito.html.twig', array(
            'idTramite'=>$idTramite,
            'historial'=>$this->historial($idTramite),
    ...
    public function derivaDistritoAction(Request $request){
        ...
        try {
            ...
            if ($gestion != $gestionActual) {
                $request->getSession()
                    ->getFlashBag()
                    ->add('error', "El Tramite ". $idTramite ." pertenece a una gestion diferente, no puede continuar. Se sugiere la finalización del tramite.");
    ...
    public function recepcionVerificaDepartamentoAction(Request $request){
        ...
        return $this->render('SieProcesosBundle:TramiteModCalVigente:formularioVistaDepartamento.html.twig', array(
            'idTramite'=>$idTramite,
            'historial'=>$historial
        ));
    }
