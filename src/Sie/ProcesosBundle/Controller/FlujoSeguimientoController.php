...
class FlujoSeguimientoController extends Controller
{
    ...
    public function verFlujoAction(Request $request )
    {
        ...
        //dump($data);die;
        if (($form['proceso'] == 5 && !$data['nombre_ie']) || ($form['proceso'] == 6 && !$data['estudiante']) || ($form['proceso'] == 7 && !$data['estudiante'])) 
        {
    ...
    public function listarF($flujotipo,$tramite)
    {
        ...
        if($flujotipo == 6 || $flujotipo == 7 )
        {
            ...
            where t1.tramite_id=". $tramite .")d ON p.id=d.flujo_proceso_id order by p.orden
            ");
        }
        $query->execute();
