...
//                    }
                    if (!$entity[0]["estadofintramite"]) {
                        ...
                        try {
                            ...
                            if ($ges > 2014) {
                                $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_Estudiantelegalizacion_Departamento_v1.rptdesign&serie=' . str_pad($serie, 7, "0", STR_PAD_LEFT) . '&gestion=' . $ges . '&&__format=pdf&'));
                            } else {
                                $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_Estudiantelegalizacion_Departamento_v1.rptdesign&serie=' . $serie . '&gestion=' . $ges . '&&__format=pdf&'));
                            }
    ...
    private function creaFormularioBuscador($routing, $value1, $value2, $value3, $identificador) {
        if ($identificador == 0) {
            $form = $this->createFormBuilder()
                    ...
                    ->add('lista', 'choice', array('label' => 'Lista',
    ...
    private function creaFormularioImpresion($routing, $value1, $value2, $value3, $identificador) {
        ...
        $form = $this->createFormBuilder()
                ->setAction($this->generateUrl($routing))
                ->add('sie', 'text', array('label' => 'SIE', 'attr' => array('value' => $value1, 'class' => 'form-control', 'pattern' => '[0-9\sñÑ]{6,8}', 'maxlength' => '8', 'autocomplete' => 'on', 'style' => 'text-transform:uppercase')))
                ->add('gestion', 'choice', array('label' => 'Lista',
                    'choices' => $aGestion,
                    'data' => $value2))
                ->add('identificador', 'hidden', array('attr' => array('value' => $identificador)))
...
//    //                        return $this->redirectToRoute($retorna);
//    //                    }
//                        if ($verificaEstudianteRude == 2){
//                            $em->getConnection()->rollback();
//                            $this->session->getFlashBag()->set('danger', array('title' => 'Error', 'message' => 'El estudiante con código '.$rude.' ya cuenta con un diploma de bachiller'));
//                            return $this->redirectToRoute($retorna);
//                        }
//
...
//                    $response->headers->set('Content-type', 'application/pdf');
//                    $response->headers->set('Content-Disposition', sprintf('attachment; filename="%s"', $arch));
//                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/jdp_lst_EstudiantesJuegosSec_Participaciones_foto_v3.rptdesign&depto=1&&__format=pdf&'));
//                    $response->headers->set('Content-Transfer-Encoding', 'binary');
//                    $response->setStatusCode(200);
//                    $response->headers->set('Pragma', 'no-cache');
//                    $response->headers->set('Expires', '0');
...
//                }
    ...
    public function printReportesDistritoPdfAction(Request $request) {
        ...
        if (count($entitySistema) > 0) {
            if ($entitySistema[0]["codigo"] == 2) {
                $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/dpl_lst_EstudiantesTramiteDistritoAlternativa_UnidadEducativa_v1.rptdesign&p_codUE=' . $sie . '&p_gestion=' . $gestion . '&&__format=pdf&'));
            } else {
                if ($estudiantes != '') {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/dpl_lst_EstudiantesTramiteDistrito_Seleccionado_v1.rptdesign&p_codUE=' . $sie . '&p_gestion=' . $gestion . '&p_ids=' . $estudiantes . '&&__format=pdf&'));
                } else {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/dpl_lst_EstudiantesTramiteDistrito_UnidadEducativa_v1.rptdesign&p_codUE=' . $sie . '&p_gestion=' . $gestion . '&&__format=pdf&'));
                }
            }
        } else {
            $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/dpl_lst_EstudiantesTramiteDistrito_UnidadEducativa_v1.rptdesign&p_codUE=' . $sie . '&p_gestion=' . $gestion . '&&__format=pdf&'));
    ...
    public function impresionListadoPdfAction(Request $request) {
        ...
        if (count($entitySistema) > 0) {
            if ($lista == 100) {
                if ($estudiantes != '') {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/dpl_lst_EstudiantesDiplomaImpreso_Seleccionado_v1.rptdesign&p_codUE=' . $sie . '&p_gestion=' . $gestion . '&p_ids=' . $listaEstudiantes . '&&__format=pdf&'));
                } else {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/dpl_lst_EstudiantesDiplomaImpreso_Todo_v1.rptdesign&p_codUE=' . $sie . '&p_gestion=' . $gestion . '&&__format=pdf&'));
                }
            } elseif ($lista == 101) {
                if ($estudiantes != '') {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/dpl_lst_EstudiantesDiplomaAnulado_Seleccionado_v1.rptdesign&p_codUE=' . $sie . '&p_gestion=' . $gestion . '&p_ids=' . $listaEstudiantes . '&&__format=pdf&'));
                } else {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/dpl_lst_EstudiantesDiplomaAnulado_Todo_v1.rptdesign&p_codUE=' . $sie . '&p_gestion=' . $gestion . '&&__format=pdf&'));
                }
            } else {
                if ($entitySistema[0]["codigo"] == 2) {
                    //$response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/dpl_lst_EstudiantesTramiteDistritoAlternativa_UnidadEducativa_v1.rptdesign&p_codUE='.$sie.'&p_gestion='.$gestion.'&&__format=pdf&'));
                    if ($estudiantes != '') {
                        $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/dpl_lst_EstudiantesTramiteBandejaAlternativa_Seleccionado_v1.rptdesign&p_codUE=' . $sie . '&p_gestion=' . $gestion . '&p_ids=' . $listaEstudiantes . '&p_tipoLista=' . $lista . '&&__format=pdf&'));
                    } else {
                        $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/dpl_lst_EstudiantesTramiteBandejaAlternativa_Todo_v1.rptdesign&p_codUE=' . $sie . '&p_gestion=' . $gestion . '&p_tipoLista=' . $lista . '&&__format=pdf&'));
                    }
                } else {
                    if ($estudiantes != '') {
                        $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/dpl_lst_EstudiantesTramiteBandeja_Seleccionado_v1.rptdesign&p_codUE=' . $sie . '&p_gestion=' . $gestion . '&p_ids=' . $listaEstudiantes . '&p_tipoLista=' . $lista . '&&__format=pdf&'));
                    } else {
                        $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/dpl_lst_EstudiantesTramiteBandeja_Todo_v1.rptdesign&p_codUE=' . $sie . '&p_gestion=' . $gestion . '&p_tipoLista=' . $lista . '&&__format=pdf&'));
                    }
                }
            }
        } else {
    ...
    public function printReportesDepartamentalPdfAction(Request $request) {
        ...
        $response->headers->set('Content-type', 'application/pdf');
        $response->headers->set('Content-Disposition', sprintf('attachment; filename="%s"', $arch));
        $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/dpl_lst_EstudiantesTramiteDepartamento_UnidadEducativa_v1.rptdesign&p_codUE=' . $sie . '&p_gestion=' . $gestion . '&&__format=pdf&'));
        $response->setStatusCode(200);
        $response->headers->set('Content-Transfer-Encoding', 'binary');
        $response->headers->set('Pragma', 'no-cache');
        $response->headers->set('Expires', '0');
    ...
    public function printDiplomaPdfAction(Request $request) {
        ...
        if ($depto == 1) {
            $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion));
        } elseif ($depto == 2) {
            $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_v3_lp.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion));
        } elseif ($depto == 3) {
            $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_v3_cba.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion));
        } elseif ($depto == 4) {
            $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_v3_tj.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion));
        } elseif ($depto == 5) {
            $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_v3_bn.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion));
        } elseif ($depto == 6) {
            $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_v3_pn.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion));
        } elseif ($depto == 7) {
            $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_v3_pt.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion));
        } elseif ($depto == 8) {
            $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_v3_or.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion));
        } elseif ($depto == 9) {
            $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_v3_scz.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion));
        } else {
            $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion));
        }
        ...
        $response->headers->set('Pragma', 'no-cache');
        $response->headers->set('Expires', '0');
        //die('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_v2.rptdesign&__format=pdf&unidadeducativa='.$ue.'&gestion_id='.$gestion);
        //$response->send();
        return $response;
    }
    ...
    public function impresionDiplomaPdfAction(Request $request) {
        ...
        switch ($gestion) {
            case 2009:
                if ($depto == 1) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2009_ch_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion));
                } elseif ($depto == 2) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2009_lp_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion));
                } elseif ($depto == 3) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2009_cba_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion));
                } elseif ($depto == 4) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2009_or_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion));
                } elseif ($depto == 5) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2009_pt_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion));
                } elseif ($depto == 6) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2009_tj_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion));
                } elseif ($depto == 7) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2009_scz_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion));
                } elseif ($depto == 8) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2009_bn_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion));
                } elseif ($depto == 9) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2009_pn_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion));
                } else {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2009_ch_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion));
                }
                break;
            case 2010:
                if ($depto == 1) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2010_ch_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 2) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2010_lp_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 3) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2010_cba_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 4) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2010_or_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 5) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2010_pt_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 6) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2010_tj_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 7) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2010_scz_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 8) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2010_bn_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 9) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2010_pn_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } else {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2010_ch_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                }
                break;
            case 2011:
                if ($depto == 1) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2011_ch_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 2) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2011_lp_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 3) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2011_cba_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 4) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2011_or_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 5) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2011_pt_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 6) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2011_tj_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 7) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2011_scz_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 8) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2011_bn_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 9) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2011_pn_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } else {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2011_ch_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                }
                break;
            case 2012 :
                if ($depto == 1) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2012_ch_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 2) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2012_lp_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 3) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2012_cba_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 4) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2012_or_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 5) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2012_pt_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 6) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2012_tj_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 7) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2012_scz_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 8) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2012_bn_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 9) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2012_pn_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } else {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2012_ch_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                }
                break;
            case 2014:
                if ($depto == 1) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2014_ch_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 2) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2014_lp_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 3) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2014_cba_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 4) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2014_or_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 5) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2014_pt_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 6) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2014_tj_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 7) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2014_scz_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 8) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2014_bn_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 9) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2014_pn_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } else {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_2014_ch_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                }
                break;
            case 2015:
                if ($depto == 1) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 2) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_v3_lp.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 3) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_v3_cba.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 4) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_v3_or.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 5) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_v3_pt.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 6) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_v3_tj.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 7) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_v3_scz.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 8) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_v3_bn.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 9) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_v3_pn.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } else {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                }
                break;
            default :
                if ($depto == 1) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_default_ch_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 2) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_default_lp_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 3) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_default_cba_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 4) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_default_or_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 5) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_default_pt_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 6) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_default_tj_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 7) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_default_scz_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 8) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_default_bn_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } elseif ($depto == 9) {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_default_pn_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                } else {
                    $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_unidadeducativa_default_ch_v2.rptdesign&__format=pdf&unidadeducativa=' . $ue . '&gestion_id=' . $gestion . '&tipo=' . $tipoImpresion));
                }
                break;
    ...
    public function printDiplomaListaPdfAction(Request $request) {
        ...
        $response->headers->set('Content-type', 'application/pdf');
        $response->headers->set('Content-Disposition', sprintf('attachment; filename="%s"', $arch));
        $response->setContent(file_get_contents('http://172.20.0.117:8080/birt-viewer/frameset?__report=siged/gen_dpl_diplomaEstudiante_numeroSerie_v1.rptdesign&__format=pdf&p_codUE=' . $ue . '&p_gestion=' . $gestion));
        $response->setStatusCode(200);
        $response->headers->set('Content-Transfer-Encoding', 'binary');
        $response->headers->set('Pragma', 'no-cache');
        $response->headers->set('Expires', '0');
    ...
    private function verificaEstudianteConDobleTramite($id, $tipo) {
        ...
        if (count($entityEst) > 0) {
            ...
            $query = $em->getConnection()->prepare("
                    ...
                    left join (select * from documento where documento_estado_id = 1 and documento_tipo = 1) as d on d.tramite_id = t.id
                    where t.esactivo = 'true' and e.codigo_rude = '" . $rude . "' and iec.nivel_tipo_id in (13,15)
                    ");
            $query->execute();
            $entity = $query->fetchAll();
    ...
    public function buscaEstudiantesUnidadEducativa($sie, $gestion, $identificador, $lista) {
        ...
        if ($identificador == 0 and $lista < 100) {
            $query = $em->getConnection()->prepare("
                ...
                where t.gestion_id = :gestion::INT and iec.institucioneducativa_id = :sie::INT
                order by td.id desc, e.paterno, e.materno, e.nombre
            ");
        }
        ...
        if ($identificador == 0 and $lista == 100) {
            $query = $em->getConnection()->prepare("
                ...
                WHERE t.esactivo = true AND t.gestion_id = :gestion::INT and iec.institucioneducativa_id = :sie::INT
                ORDER BY d.id desc, e.paterno, e.materno, e.nombre
            ");
        }
            ...
            $query = $em->getConnection()->prepare("
                        ...
                        inner join (
                            select * from tramite_detalle where id in (
                                ...
                                where tramite_estado_id <> 4 and insc.institucioneducativa_id = :sie::INT group by trad.tramite_id
                            )
                        ...
                        left join lugar_tipo as lt1 on lt1.id = lt.lugar_tipo_id
                        left join pais_tipo as pt on pt.id = e.pais_tipo_id
                        where td.flujo_proceso_id = 2 and t.gestion_id = :gestion::INT and iec.institucioneducativa_id = :sie::INT
        ...
        if ($identificador == 15) {
            $query = $em->getConnection()->prepare("
                        ...
                        left join lugar_tipo as lt1 on lt1.id = lt.lugar_tipo_id
                        left join pais_tipo as pt on pt.id = e.pais_tipo_id
                        where td.flujo_proceso_id = 3 and t.gestion_id = :gestion::INT and iec.institucioneducativa_id = :sie::INT
                        order by e.paterno, e.materno, e.nombre
                    ");
        }
        /*
        ...
        if ($identificador == 16) {
            $query = $em->getConnection()->prepare("
                        ...
                        left join lugar_tipo as lt1 on lt1.id = lt.lugar_tipo_id
                        left join pais_tipo as pt on pt.id = e.pais_tipo_id
                        where td.flujo_proceso_id = 4 and t.gestion_id = :gestion::INT and iec.institucioneducativa_id = :sie::INT
                        order by e.paterno, e.materno, e.nombre
                    ");
        }
        /*
        ...
        if ($identificador == 17) {
            $query = $em->getConnection()->prepare("
                        ...
                        left join lugar_tipo as lt1 on lt1.id = lt.lugar_tipo_id
                        left join pais_tipo as pt on pt.id = e.pais_tipo_id
                        where td.flujo_proceso_id = 5 and t.gestion_id = :gestion::INT and iec.institucioneducativa_id = :sie::INT
                        order by e.paterno, e.materno, e.nombre
                    ");
        }
        /*
        ...
        if ($identificador == 18) {
            $query = $em->getConnection()->prepare("
                        ...
                        left join lugar_tipo as lt1 on lt1.id = lt.lugar_tipo_id
                        left join pais_tipo as pt on pt.id = e.pais_tipo_id
                        where td.flujo_proceso_id = 6 and t.gestion_id = :gestion::INT and iec.institucioneducativa_id = :sie::INT
                        order by e.paterno, e.materno, e.nombre
                    ");
        }
        $query->bindValue(':sie', $sie);
    ...
    public function indexAction(Request $request) {
        ...
        $query = $repository->createQueryBuilder('t')
            ...
            ->andWhere('fp.id in (:flujoP)')
            //->andWhere('td.tramiteEstado in (:estados)')
            ->setParameter('idInstitucion', $this->session->get('ie_id'))
            ->setParameter('flujoP', array(11,12,13,14))
            //->setParameter('estados', array(1,2,3,4))
            ->orderBy('t.gestionId, fp.id')
            ->getQuery();

        $entities = $query->getResult();

        //dump($entities);die;
        return $this->render($this->session->get('pathSystem') . ':Tramite:search.html.twig', array(
                    'form' => $this->searchForm($arrSieInfo)->createView(),
                    'arrSieInfo'=>$arrSieInfo,
                    'entities' => $entities
        ));
    }

    public function indexTsieAction(Request $request) {
        ...
        switch($usuario_rol){
            case 10://distrito
                $query = $repository->createQueryBuilder('t')
                    ...
                    ->where('jg.lugarTipoIdDistrito = :distrito')
                    ->andWhere('fp.id in (:flujoP)')
                    ->setParameter('distrito', $lugar->getId())
                    ->setParameter('flujoP', array(12,13,14))
                    ->getQuery();
                $verificar = $query->getResult();//tabla contraste
                ...
                if($verificar){
                    $query = $repository->createQueryBuilder('t')
                        ...
                        ->andWhere('t.id not in (:tramiteV)')
                        ->setParameter('distrito', $lugar->getId())
                        ->setParameter('flujoP', 11)
                        ->setParameter('proceso', 9)
                        ->setParameter('tramiteV', $verificar)
                        ->orderBy('ie.id, t.gestionId')
                        ->getQuery();
                } else {
                    $query = $repository->createQueryBuilder('t')
                        ->select('td')
            ...
            case 7://departamento
                ...
                $distritos = $em->getRepository('SieAppWebBundle:LugarTipo')->findBy(array('lugarTipo' => $lugar->getId()));
                $distritosArray = array();
                foreach ($distritos as $d) {
                    $distritosArray[$d->getId()] = $d->getId();
                }
                ...
                $query = $repository->createQueryBuilder('t')
                    ...
                    ->innerJoin('SieAppWebBundle:Institucioneducativa', 'ie', 'WITH', 't.institucioneducativa = ie.id')
                    ->innerJoin('SieAppWebBundle:JurisdiccionGeografica', 'jg', 'WITH', 'ie.leJuridicciongeografica = jg.id')
                    ->where('jg.lugarTipoIdDistrito in (:distritos)')
                    ->andWhere('fp.id in (:flujoP)')
                    ->setParameter('distritos', $distritosArray)
                    ->setParameter('flujoP', array(13,14))
                    ->getQuery();
                $verificar = $query->getResult();//tabla contraste
                ...
                if($verificar){
                    $query = $repository->createQueryBuilder('t')
                        ...
                        ->andWhere('t.id not in (:tramiteV)')
                        ->setParameter('distritos', $distritosArray)
                        ->setParameter('flujoP', 12)
                        ->setParameter('proceso', 10)
                        ->setParameter('tramiteV', $verificar)
                        ->orderBy('ie.id, t.gestionId')
                        ->getQuery();
                } else {
            ...
            case 8://nacional
                $query = $repository->createQueryBuilder('t')
                    ...
                    ->andWhere('fp.id in (:flujoP)')
                    ->setParameter('flujoP', array(11,12,13,14))
                    ->orderBy('ie.id, t.gestionId, fp.id')
                    ->getQuery();
                break;
            default://otros
                $query = $repository->createQueryBuilder('t')
                    ...
                    ->andWhere('fp.id = :flujoP')
                    ->andWhere('fp.proceso = :proceso')
                    ->setParameter('flujoP', 14)
                    ->setParameter('proceso', 12)
                    ->orderBy('ie.id, t.gestionId')
                    ->getQuery();
                break;
        }
        ...
        //dump($entities);die;
        return $this->render($this->session->get('pathSystem') . ':Tramite:list_tramite.html.twig', array(
                    'entities' => $entities,
                    'lugar' => $lugar
        ));
    }
    ...
    /**
     * Crea un formulario para generar una solicitud
     *
     */
    private function searchForm($arrSieInfo) {
        ...
        $query = $em->createQuery(
                        'SELECT t FROM SieAppWebBundle:TramiteTipo t
                        WHERE t.id IN (:id) ORDER BY t.id')
                ->setParameter('id', array(3,19,20,21,22));

        $tipos = $query->getResult();
        $tiposArray = array();
        foreach ($tipos as $t) {
            $tiposArray[$t->getId()] = $t->getTramiteTipo();
        }

        $form = $this->createFormBuilder()
                ->setAction($this->generateUrl('sie_herramienta_tramite_recepcion_distrito_registra'))
                ->add('idInstitucion', 'hidden', array('data' => $arrSieInfo['id']))
                ->add('gestion', 'choice', array('label' => 'Gestión', 'required' => true, 'choices' => $gestiones, 'attr' => array('class' => 'form-control')))
                //->add('obs', 'text', array('label' => 'Observación', 'required' => false, 'attr' => array('class' => 'form-control')))
                ->add('tipo', 'choice', array('label' => 'Tipo de trámite', 'required' => true, 'choices' => $tiposArray, 'attr' => array('class' => 'form-control')))
                ->add('buscar', 'submit', array('label' => 'Enviar Solicitud', 'attr' => array('class' => 'btn btn-primary')))
                ->getForm();
        return $form;
    }
    ...
    public function resultAction(Request $request) {
        ...
        //dump($personas);die;
        return $this->render($this->session->get('pathSystem') . ':Tramite:result.html.twig', array(
                    'personas' => $personas,
    ...
    public function registraTramiteAcademicoAction(Request $request) {
        /*
         * Define la zona horaria y halla la fecha actual
         */
        date_default_timezone_set('America/La_Paz');
        ...
        if ($request->isMethod('POST')) {
            ...
            $gestion = $form['gestion'];
            $ue = $form['idInstitucion'];
            $tipo = $form['tipo'];
            $obs = '';

            switch($tipo) {
                case 22: $ihttipo = 1; break;
                case 19: $ihttipo = 2; break;
                case 20: $ihttipo = 3; break;
                case 21: $ihttipo = 6; break;
                default: $ihttipo = 5; break;
            }

            //verificar - crear registro en InstitucioneducativaHumanisticoTecnico
            $ieducativa = $em->getRepository('SieAppWebBundle:Institucioneducativa')->findOneById($ue);
            ...
            if(!$humanistico_tecnico) {
                ...
                $humanistico_tecnico->setGestionTipoId($gestion);
            ...
            $query = $repository->createQueryBuilder('t')
                ...
                ->andWhere('td.tramiteEstado in (:estados)')
                ->setParameter('idInstitucion', $ue)
                ->setParameter('gestion', $gestion)
                ->setParameter('estados', array(1,3,4))
                ->orderBy('t.gestionId')
                ->getQuery();
            ...
            if($entities){
                return $this->redirect($this->generateUrl('sie_herramienta_tramite_index'));
            }
            //dump($entities);die;
            ...
            try {
                ...
                $entityTramite->setInstitucioneducativa($entityInstitucioneducativa);
                ...
                /*
                 * Extrae el id del registro ingresado de la tabla tramite
                 */
                ...
                $this->get('session')->getFlashBag()->add('success', '¡Correcto! Trámite registrado.');
                $this->session->set('datosBusqueda', array('sie' => $ue, 'gestion' => $gestion));

                return $this->redirectToRoute('sie_herramienta_tramite_index');
            } catch (Exception $e) {
                $em->getConnection()->rollback();
                $this->get('session')->getFlashBag()->add('danger', '¡Error! Dificultades al ingresar registro, intente nuevamente.');
                return $this->redirectToRoute('sie_herramienta_tramite_index');
            }
            $valores = array('sie' => $ue, 'gestion' => $gestion);
            //return $this->redirect($this->generateUrl('sie_diploma_tramite_busca_ue', array('valores'=> $valores,)));
        } else {
            $this->get('session')->getFlashBag()->add('danger', '¡Error! Datos inválidos, intente nuevamente.');
            $valores = array('sie' => 0, 'gestion' => 0);
            return $this->redirectToRoute('sie_herramienta_tramite_index');
        }
    }

    public function registraTramiteAcademicoApruebaAction(Request $request) {
        /*
         * Define la zona horaria y halla la fecha actual
         */
        date_default_timezone_set('America/La_Paz');
        ...
        if ($request->isMethod('POST')) {
            ...
            try {
                ...
                return $this->redirectToRoute('sie_herramienta_tramite_tsie_index');
            } catch (Exception $e) {
                $em->getConnection()->rollback();
                $this->get('session')->getFlashBag()->add('danger', '¡Error! Dificultades al ingresar registro, intente nuevamente.');
                return $this->redirectToRoute('sie_herramienta_tramite_tsie_index');
    ...
    public function validacionueAction(Request $request) {
        ...
        $registroConsol = $em->getRepository('SieAppWebBundle:RegistroConsolidacion')->findOneBy(array('unidadEducativa' => $form['idInstitucion'], 'gestion' => $form['gestion']));
        ...
        if($registroConsol){
            ...
            $rconsol->setUnidadEducativa($form['idInstitucion']);
            $rconsol->setTabla('**');
        ...
        if ($this->session->get('ue_modular')) {
            ...
            if ($registroConsol->getBim2() == 0) {
              $registroConsol->setBim2('2');
              $em->persist($registroConsol);
              $em->flush();
            }
            if ($registroConsol->getBim3() == 0) {
        ...
        if($inconsistencia) {
            return $this->render($this->session->get('pathSystem') . ':Tramite:list_inconsistencia.html.twig', array('inconsistencia' => $inconsistencia, 'institucion' =>  $form['idInstitucion'], 'gestion' => $form['gestion'], 'periodo' => $periodo));
        }
        else {
            $registroConsol = $em->getRepository('SieAppWebBundle:RegistroConsolidacion')->findOneBy(array('unidadEducativa' => $form['idInstitucion'], 'gestion' => $form['gestion']));
    ...
    public function eliminartramiteueAction(Request $request) {
        ...
        $tramite = $em->getRepository('SieAppWebBundle:Tramite')->findOneBy(array('id' => $form['tramiteId']));
        $tramitedetalle = $em->getRepository('SieAppWebBundle:TramiteDetalle')->findBy(array('tramite' => $tramite));

        dump($tramite);dump($tramitedetalle);die;*/
        ...
        if ($request->isMethod('POST')) {
            ...
            try {
                ...
                $this->get('session')->getFlashBag()->add('success', '¡Correcto! Trámite anulado.');
                $this->session->set('datosBusqueda', array('sie' => $ue, 'gestion' => $gestion));

                return $this->redirectToRoute('herramienta_inbox_index');
            } catch (Exception $e) {
                $em->getConnection()->rollback();
                $this->get('session')->getFlashBag()->add('danger', '¡Error! Dificultades al anular registro, intente nuevamente.');
                return $this->redirectToRoute('herramienta_inbox_index');
            }
            $valores = array('sie' => $ue, 'gestion' => $gestion);
            //return $this->redirect($this->generateUrl('sie_diploma_tramite_busca_ue', array('valores'=> $valores,)));
