...
class RegularizacionRueController extends Controller {
            ...
            if ($aTuicion[0]['get_ue_tuicion']) {
                ...
                if($institucionHabilitada) {
                    if($institucionHabilitada->getEsactivo()){
                        $this->get('session')->getFlashBag()->add('noSearch', 'La unidad educativa ya realizó la actualización de "Fecha de Fundación".');
                        return $this->redirect($this->generateUrl('regularizacion_rue_index'));
                    } else {
                        return $this->render($this->session->get('pathSystem') . ':RegularizacionRue:result.html.twig', array(
    ...
    private function formFechaFundacion($sie) {
        $form = $this->createFormBuilder()
                ->setAction($this->generateUrl('regularizacion_rue_save'))
            ...
            if ($aTuicion[0]['get_ue_tuicion']) {
                ...
                if($institucionHabilitada) {
                    if($institucionHabilitada->getEsactivo()){
                        ...
                        if(!empty($documento)){
                            $root_rue_path = $this->get('kernel')->getRootDir() . '/../web/uploads/archivos/rue/'.$form['sie'];
                            
                            if (!file_exists($root_rue_path)) {
                                mkdir($root_rue_path, 0777);
                            }
                            $destination_path = $this->get('kernel')->getRootDir() . '/../web/uploads/archivos/rue/'.$form['sie'];
                            ...
                            $institucionHabilitada->setFechaFundacion(new \DateTime($form['fechaFundacion']));
                            ...
                            return $this->render($this->session->get('pathSystem') . ':RegularizacionRue:saved.html.twig', array(
