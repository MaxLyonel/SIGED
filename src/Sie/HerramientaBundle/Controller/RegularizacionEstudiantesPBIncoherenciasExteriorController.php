...
class RegularizacionEstudiantesPBIncoherenciasExteriorController extends Controller
{
	...
	public function __construct()
	{
		...
		$this->path = '/../web/uploads/archivos/regularizacion/estudiantes_post_bachillerato_exterior/';//Esto es una sugerencia y debe ser socializado con el lic Carlos
	...
	public function getRegularizacionRegistrarHistorialAction(Request $request)//DEPARTAMENTAL
	{
		...
		return $this->render($this->session->get('pathSystem') . ':Regularizacion_EstudiantesPBIncoherenciaExterior:departamental_regularizarHistorial.html.twig',array(
			'flujo_id' => $request_id,
	...
	
	{
		...
		$request_fileDocsPasaporte 		= $_FILES["request_pasaporte_ci"];
		// $request_fileDocsInteresado     = $_FILES["request_fileInforme"];
		$request_fileDocsCertificadoNacimiento 	= $_FILES["request_certificado_nac"];
		$request_fileDocsHistorialAcademico	    = $_FILES["request_historial_academico"];
		...
		$estudiante=$qb->select('e')
			 ...
			 ->setParameters(array('carnetIdentidad'=>$ci))
			 ->getQuery()
			 ->getSingleResult();
		...
		if( $estudiante && $request_flujo_id>0 && $request_distrito>0 && ( !$tieneErrores && count($inscripciones_procesada)>0 && count($inscripciones_procesada)<=$NRO_MAX_INSCRIPCIONES)  && isset($request_fileInforme ) && isset($request_fileDocsPasaporte)&& isset($request_fileDocsCertificadoNacimiento)&& isset($request_fileDocsHistorialAcademico))
		{
			$em 			= $this->getDoctrine()->getManager();
			$tramite_id 	= -1;

			$ci 			= $estudiante->getCarnetIdentidad();
			$path=$this->path;
			...
			if($ci != null && !empty($ci))
			{
				...
				try
				{
					...
					if( $cantidadTramitesRegularizacion < $NRO_MAX_INSCRIPCIONES )
					{
						...
						//generar nombre aleatorio en base a los datos del estudiante creando una funcion  y concatenar con funciones tramie
						
						$estudiante_identificador = $this->generarIDSolicitante($estudiante);
						list($archivoGuardado1,$nombre1) 	= $this->guardarArchivo($request_fileInforme,$tramite_id,$ci,$path,'Informe');
						list($archivoGuardado2,$nombre2) 	= $this->guardarArchivo($request_fileDocsPasaporte,$tramite_id,$ci,$path,'Pasaporte');
						list($archivoGuardado3,$nombre3) 	= $this->guardarArchivo($request_fileDocsCertificadoNacimiento,$tramite_id,$ci,$path,'CertificadoNacimiento');
						list($archivoGuardado4,$nombre4) 	= $this->guardarArchivo($request_fileDocsHistorialAcademico,$tramite_id,$ci,$path,'HistorialAcademico');

						//Realizamos la verificacion de si los archvios fueron guardados
						if($archivoGuardado1 == true && $archivoGuardado2 == true && $archivoGuardado3 == true && $archivoGuardado4 == true)
						{
							//$inscripciones 				= json_decode($request_inscriptions);
							$informes 									= array(array('informe' =>$nombre1));
							$pasaporte									= array(array('Pasaporte' => $nombre2));
							$certificado								= array(array('CertificadoNacimiento' =>$nombre3));
							$historialacademico 						= array(array('HistorialAcademico' =>$nombre4));

							$datosJSONInscripcion 			= array(
								'estudiante'		=> $estudiante->getCarnetIdentidad(),
								'inscripciones' 	=> $inscripciones,
								'informes' 			=> $informes,
								'pasaporte' 						=> $pasaporte,
								'CertificadoNacimiento' 			=> $certificado,
							...
							$tipoTramite = $em->getRepository('SieAppWebBundle:TramiteTipo')->findOneBy(array('obs'=>'EPBIE'));
								...
								if($registroTramite)
								{
									if($registroTramite['dato']===true && $registroTramite['tipo']==='exito')
									{
										...
										if ($renombrado)
										{
											...
											$urlReporte 	= $this->generateUrl('regularizacion_estudiantesPostBachilleratoIncoherenciasExterior_reporteDepartamental',array('idtramite' => $registroTramite['idtramite'])) ;
										}
										else
										{
											$data=NULL;
											...
											$this->borrarArchivo($path,$nombre3,$tramite_id);
											$this->borrarArchivo($path,$nombre4,$tramite_id);
											$status = 404;
			...
			else
			{
				$data 	= NULL;
				$status = 404;
				$msj 	= 'Ocurrio un error al guardar el registro, el estudiante no tiene Carnet de Identidad.';
			}
		}
		else
		{
	...
	{
		/*Obtenemos los datos del formulario CON OTROS DATOS*/
		$em 		= $this->getDoctrine()->getManager();
		$form 		= $request->request->all();
		$nombre=filter_var($form['nombre'],FILTER_SANITIZE_SPECIAL_CHARS);
		$paterno=filter_var($form['paterno'],FILTER_SANITIZE_SPECIAL_CHARS);
		$materno=filter_var($form['materno'],FILTER_SANITIZE_SPECIAL_CHARS);
		...
		$estudiante=$qb->select('e')
			...
			//  ->orWhere('e.codigoRude = :codigoRude')
			 ...
			 ->setParameters(array('carnetIdentidad'=>$ci_rude,'nombre'=>$nombre,'paterno'=>$paterno,'materno'=>$materno))
		...
		/*Obtenemos los datos del formulario SOLO CARNET*/
		$em 		= $this->getDoctrine()->getManager();
		$form 		= $request->request->all();
		...
		$estudiante=$qb->select('e')
			 ->from('SieAppWebBundle:Estudiante', 'e')
			 ->where('e.carnetIdentidad = :carnetIdentidad')
			 ->setParameters(array('carnetIdentidad'=>$ci_rude,))
		...
		return $this->render($this->session->get('pathSystem') . ':Regularizacion_EstudiantesPBIncoherenciaExterior:buscarEstudiante.html.twig',array(
	...
	public function regularizacionCrearInscripcionAction(Request $request)//BORRAR ESTA FUNCION, NO SERAN UTILIZADAS
	{
		...
		if($saved)
		{
			$data=$this->renderView($this->session->get('pathSystem') . ':Regularizacion_EstudiantesPBIncoherenciaExterior:Fragmentos/frag_asignacionMaterias.html.twig',array('nueva_inscripcion'=>$newInscripcion,));
			$status=200;
		}
		else
		{
	...
	public function regularizacion_gestionesPasadas_getMateriasAction($id)
	{
		...
		{
			$data=$this->renderView($this->session->get('pathSystem') . ':Regularizacion_EstudiantesPBIncoherenciaExterior:busquedaMaterias.html.twig',
				array
				(
	...
	public function getRegularizacionSolicitudesRecibidasDepartamentalAction(Request $request)
	{
		...
		return $this->render($this->session->get('pathSystem') . ':Regularizacion_EstudiantesPBIncoherenciaExterior:solicitudesRecibidas.html.twig',array(
			'inscripciones'=>$inscripciones,
			...
			'url' => 'regularizacion_estudiantesPostBachilleratoIncoherenciasExterior_getSolicitudesDepartamental',
		));
	}
	...
	public function postRegularizacionDetalleSolicitudesRecibidasDepartamentalPorConcluirAction(Request $request)
	{
		...
		if($tramite_id>0)
		{
			...
			if($tramite)
			{
				...
				if(strlen($datosJSONInscripcion)>0)
				{
					try
					{
						...
						$ci=property_exists($datosJSONInscripcion_decode,'estudiante')?$datosJSONInscripcion_decode->estudiante:-1;

						$qb = $em->createQueryBuilder();
						$estudiante=$qb->select('e')
						 ->from('SieAppWebBundle:Estudiante', 'e')
						 ->where('e.carnetIdentidad = :ci')
						 ->setParameters(array('ci'=>$ci))
						 ->getQuery()
						 ->getSingleResult();
						...
						if($estudiante)
						{
							if( $datosJSONInscripcion_decode && property_exists($datosJSONInscripcion_decode,'inscripciones'))
							{
								...
								if($tieneErrores==false && $institucioneducativa)
								{
									...
									$adjuntosDocsInteresado=array();
									if($inscripcionesDecode  && property_exists($inscripcionesDecode,'pasaporte'))
									{
										//continuamos con los documentos del interesado
										$pasaporte = $inscripcionesDecode->pasaporte;
									}
									$adjuntosDocsInteresado=array();
									if($inscripcionesDecode  && property_exists($inscripcionesDecode,'CertificadoNacimiento'))
									{
										//continuamos con los documentos del interesado
										$CertificadoNacimiento = $inscripcionesDecode->CertificadoNacimiento;
									}
									$adjuntosDocsInteresado=array();
									if($inscripcionesDecode  && property_exists($inscripcionesDecode,'HistorialAcademico'))
									{
										//continuamos con los documentos del interesado
										$HistorialAcademico = $inscripcionesDecode->HistorialAcademico;
									}
			...
			return $this->render($this->session->get('pathSystem') . ':Regularizacion_EstudiantesPBIncoherenciaExterior:departamental_detalleSolicitudesRecibidasPorConcluir.html.twig',array(
				'estudiante'=>$estudiante,
				'path' => $this->path,
				'error' => $error,
		...
		{
			return $this->redirect($this->generateUrl('regularizacion_estudiantesPostBachilleratoIncoherenciasExterior_getSolicitudeRecibidasUE'));
		}
	}
		...
		if($tramite && ($request_procede>=0 && $request_procede<=1) )//verificamos que exista el trámite y la unidad educativa
		{
			...
			if(strlen($datosJSONInscripcion)>0 )//verificamos que el JSon existe
			{
				try //al decodificar la cadena JSON nos aseguramos que sea correcta
				{
					...
					//verificamos que exista el estudiante
					if($estudiante && $estudiante->getCarnetIdentidad())
					{
						if( $datosJSONInscripcion_decode && property_exists($datosJSONInscripcion_decode,'inscripciones'))
						{	
							...
							{
								$varevaluacion='SI';
								list($archivoGuardado,$nombre) = $this->guardarArchivo($request_fileInforme,$tramite_id,$estudiante->getCarnetIdentidad(),$this->path,'Informe-Departamental-2');
							...
							if(($varevaluacion==='NO' && strlen($request_observacion)>0) || ($varevaluacion==='SI' && $archivoGuardado === true && $datosActualizados===true))
							{
								...
								if($registroTramite)
								{
									...
									if($registroTramite['dato']==true && $registroTramite['tipo']=='exito')
									{
										...
										//No olvidar este paso aqui el error
										...
										if($request_procede==1 && $determinarAprobacionOReprobacionSolicitud==true)// procede
											$urlCertificacionConclusionEstudios = $this->generateUrl('regularizacion_estudiantesPostBachilleratoIncoherenciasExterior_reporteDepartamentalPorConcluirConclusionEstudios',array('idtramite' => $registroTramite['idtramite'])) ;
										else
											$urlCertificacionConclusionEstudios = NULL;
									}
									else
	...
	public function getRegularizacionSolicitudesRecibidasDistritalAction()
	{
		...
		return $this->render($this->session->get('pathSystem') . ':Regularizacion_EstudiantesPBIncoherenciaExterior:solicitudesRecibidas.html.twig',array(
			...
			'url'=>'regularizacion_estudiantesPostBachilleratoIncoherenciasExterior_getSolicitudesDistrital',
		));
	}
	...
	public function postRegularizacionDetalleSolicitudesRecibidasDistritalAction(Request $request)
	{
		...
		if($tramite_id>0)
		{
					...
					try
					{
						...
						if($estudiante)
						{
							if( $datosJSONInscripcion_decode && property_exists($datosJSONInscripcion_decode,'inscripciones'))
							{
								...
								$pasaporte 					= $datosJSONInscripcion_decode->pasaporte;
								$CertificadoNacimiento 		= $datosJSONInscripcion_decode->CertificadoNacimiento;
								$HistorialAcademico 		= $datosJSONInscripcion_decode->HistorialAcademico;
							}
							else
							{
								$error = 'Ocurrio un error debido a que no existen datos de las inscripciones, por favor verifique que el trámite sea correcto.';
			...
			return $this->render($this->session->get('pathSystem') . ':Regularizacion_EstudiantesPBIncoherenciaExterior:distrital_detalleSolicitudesRecibidas.html.twig',array(
				'estudiante'=>$estudiante,
				'path' => $this->path,
				'error' => $error,
				'tablaInscripciones' => $tablaInscripciones,
		...
		{
			return $this->redirect($this->generateUrl('regularizacion_estudiantesPostBachilleratoIncoherenciasExterior_getSolicitudeRecibidasDistrital'));
		}
	}
	...
	public function postRegularizacionDetalleSolicitudesRecibidasDistritalPorConcluirAction(Request $request)
	{
		...
		if($tramite_id>0)
		{
			...
			return $this->render($this->session->get('pathSystem') . ':Regularizacion_EstudiantesPBIncoherenciaExterior:distrital_detalleSolicitudesRecibidasPorConcluir.html.twig',array(
				'estudiante'=>$estudiante,
				'path' => $this->path,
				'error' => $error,
				'tablaInscripciones' => $tablaInscripciones,
	...
	private function obtenerNombresDeInscripciones($inscripcionesDecode)
	{
		...
		{
			if( $inscripcionesDecode && property_exists($inscripcionesDecode,'inscripciones') && property_exists($inscripcionesDecode,'pasaporte')&& property_exists($inscripcionesDecode,'CertificadoNacimiento')&& property_exists($inscripcionesDecode,'HistorialAcademico') && property_exists($inscripcionesDecode,'informes') && property_exists($inscripcionesDecode,'sie'))
			{
		...
		if( $sie>0 && $tramite && $institucionEducativa )
		{
			...
			if(strlen($datosJSONInscripcion)>0)
			{
				...
				if(isset($request_fileInforme))
				{
					...
					{
						//decodificamos el json para volverlo en objeto y poder procesarlos
						list($archivoGuardado,$nombre) = $this->guardarArchivo($request_fileInforme,$tramite_id,$ci,$this->path,'Informe-Distrital-1');
			...
			if(strlen($datosJSONInscripcion)>0)//verificamos que el JSon existe
			{
				...
				if(isset($request_fileInforme))
				{
					try //al decodificar la cadena JSON nos aseguramos que sea correcta
					{
						...
						{
							list($archivoGuardado,$nombre) = $this->guardarArchivo($request_fileInforme,$tramite_id,$estudiante->getCarnetIdentidad(),$this->path,'Informe-Distrital-2');
	...
	public function getRegularizacionSolicitudesRecibidasUEAction()
	{
		...
		return $this->render($this->session->get('pathSystem') . ':Regularizacion_EstudiantesPBIncoherenciaExterior:solicitudesRecibidas.html.twig',array(
			...
			'url'=>'regularizacion_estudiantesPostBachilleratoIncoherenciasExterior_getSolicitudesUE',
		));
	}
	...
	public function postRegularizacionDetalleSolicitudesRecibidasUEAction(Request $request)
	{
		...
		if($tramite_id>0)
		{
			...
			return $this->render($this->session->get('pathSystem') . ':Regularizacion_EstudiantesPBIncoherenciaExterior:ue_detalleSolicitudesRecibidas.html.twig',array(
				'estudiante'=>$estudiante,
				'path' => $this->path,
				'error' => $error,
				'tablaInscripciones' => $tablaInscripciones,
				...
				if(count($request_materias)>0 && isset($request_fileInforme))
				{
					...
					try 
					{
						...
						if($estudiante )
						{
							...
							{
								list($archivoGuardado,$nombre) = $this->guardarArchivo($request_fileInforme,$tramite_id,$estudiante->getCarnetIdentidad(),$this->path,'Informe-UE');
									...
									if($error==false)
									{
										...
										if($registroTramite)
										{
											if($registroTramite['dato']===true && $registroTramite['tipo']==='exito')
											{
												...
												$status 			= 200;
												$msj 				= $registroTramite['msg'];
												$urlReporte 		= $this->generateUrl('regularizacion_estudiantesPostBachilleratoIncoherenciasExterior_reporteUE',array('idtramite' => $registroTramite['idtramite'])) ;
												$urlActaSupletoria 	= $this->generateUrl('regularizacion_estudiantesPostBachilleratoIncoherenciasExterior_reporteUEActaSupletoria',array('idtramite' => $registroTramite['idtramite'])) ;
											}
											else
											{
												$this->borrarArchivo($this->path,$nombre,$tramite_id);
	...
	public function getSolicitudes($unidad,$tipo,$form_url)
	{
		...
		return $this->render($this->session->get('pathSystem') . ':Regularizacion_EstudiantesPBIncoherenciaExterior:datosSolicitudeRecibidas.html.twig',array(
	...
	{
		$form_url = 'regularizacion_estudiantesPostBachilleratoIncoherenciasExterior_postDetalleSolicitudesRecibidasDepartamentalPorConcluir';
		return $this->getSolicitudes(0,$tipo,$form_url);
	}
	public function getSolicitudesDistritalAction(Request $request,$tipo)
	{
		if($tipo==3)
			$form_url = 'regularizacion_estudiantesPostBachilleratoExt_postDetalleSolicitudeRecibidasDistritalPorConcluir';
		else
			$form_url = 'regularizacion_estudiantesPostBachilleratoExterior_postDetalleSolicitudeRecibidasDistrital';
		return $this->getSolicitudes(1,$tipo,$form_url);
	}
	public function getSolicitudesUEAction(Request $request,$tipo)
	{
		$form_url = 'regularizacion_estudiantesPostBachilleratoIncoherenciasExterior_postDetalleSolicitudeRecibidasUE';
	...
	private function getEstudianteInscripcion($estudiante_rude,$institucioneducativaCurso,$em)
	{
		...
		$estudiante = $em->getRepository('SieAppWebBundle:Estudiante')->findOneBy(array('carnetIdentidad'=>$estudiante_rude));
	...
	public function createEstudianteInscripcion($estudiante,$institucioneducativaCurso,$em)
	{
		...
		if($institucioneducativaCurso && $institucioneducativaCurso->getInstitucioneducativa())
		{
			...
			//HAY QUE VERIFICAR ESTO DEL ESTADO DE LA MATRICULA se creara la matricula ID (100) con estadomatricula:PROMOVIDO POR RGP-PB-RPM 265/2021
	...
	function generarIDSolicitante($estudiante){
		$retornar = null;
		if($estudiante){
	...
	function limpiarDatos($string){
		$retornar = '';
		if($string){
			$retornar = strtolower(trim($string));
		}
		return $retornar;
	}
	...
	public function reporteDepartamentalAction(Request $request,$idtramite)
	{
		...
		if($tramite_id>0)
		{
					...
					try
					{
						...
						if($estudiante)
						{
							if( $datosJSONInscripcion_decode && property_exists($datosJSONInscripcion_decode,'inscripciones'))
							{
								...
								$html= 
								$this->render($this->session->get('pathSystem') . ':Regularizacion_EstudiantesPBIncoherenciaExterior:Reportes/departamental_reporte.html.twig',array(
									'estudiante'=>$estudiante,
									'path' => $this->path,
									'error' => $error,
									'tablaInscripciones' => $tablaInscripciones,
			...
			return $this->render($this->session->get('pathSystem') . ':Regularizacion_EstudiantesPBIncoherenciaExterior:Reportes/departamental_reporte.html.twig',array(
				'estudiante'=>$estudiante,
				'path' => $this->path,
				'error' => $error,
				'tablaInscripciones' => $tablaInscripciones,
						...
						if($estudiante)
						{
							if( $datosJSONInscripcion_decode && property_exists($datosJSONInscripcion_decode,'inscripciones'))
							{
								...
								if($tieneErrores==false && $institucioneducativa)
								{
									...
									$html= 
									$this->render($this->session->get('pathSystem') . ':Regularizacion_EstudiantesPBIncoherenciaExterior:Reportes/ue_reporte.html.twig',array(
										'estudiante'=>$estudiante,
										'path' => $this->path,
										'error' => $error,
										'tablaInscripciones' => $tablaInscripciones,
						...
						if($estudiante)
						{
							if( $datosJSONInscripcion_decode && property_exists($datosJSONInscripcion_decode,'inscripciones'))
							{
								...
								if($tieneErrores==false && $institucioneducativa)
								{
									...
									$status = 200;
									$arch           = 'ACTA_SUPLETORIA-'.$tramite_id.'-'.$estudiante->getCarnetIdentidad().'.pdf';
									$response       = new Response();
									$response->headers->set('Content-type', 'application/pdf');
									$response->headers->set('Content-Disposition', sprintf('attachment; filename="%s"', $arch));
							...
							if( $datosJSONInscripcion_decode && property_exists($datosJSONInscripcion_decode,'inscripciones'))
							{
								...
								if($tieneErrores==false && $institucioneducativa)
								{
									...
									if($determinarAprobacionOReprobacionSolicitud==true)
									{
										...
										$pdf='http://127.0.0.1:57397/viewer/frameset?__report=D%3A\workspaces\reportesgp4\reg_est_cert_cal_solicitud_tramite_cert_conclu_V1_eea.rptdesign&__format=pdf'.'&tramite_id='.$tramite_id;
										$status = 200;
									}
									$arch           = 'CERTIFICADO_DE_CONCLUSIÓN_DE_ESTUDIOS-'.$tramite_id.'-'.$estudiante->getCarnetIdentidad().'.pdf';
									$response       = new Response();
									$response->headers->set('Content-type', 'application/pdf');
									$response->headers->set('Content-Disposition', sprintf('attachment; filename="%s"', $arch));
									$response->setContent(file_get_contents($pdf));
