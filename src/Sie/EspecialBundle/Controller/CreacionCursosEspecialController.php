...
//             				}
            ...
            $form = $this->createFormBuilder()
                    ...
                    ->add('area', 'choice',array('label'=>'Area','choices'=>$areas , 'empty_value' => 'Seleccionar...', 'required' => true, 'attr' => array('class' => 'form-control')))
                    ...
                    ->add('tecnica','choice',array('label'=>'Tecnica','choices'=>$tecnicas,'data'=> $tecnica,'attr'=>array('class'=>'form-control')))
                    ->add('nivelTecnico','choice',array('label'=>'Nivel de Formación Tecnica','choices'=>$nivelesTecnicoArray,'data'=> $nivelTecnico,'attr'=>array('class'=>'form-control')))
...
//             		'especialTecnicaEspecialidadTipo'=>$form['tecnica']));
    	...
    	if ($area == "1" ) {
    		$query = $em->createQuery(
    				'SELECT n.id, n.nivel FROM SieAppWebBundle:NivelTipo n
                                    WHERE n.id IN (:id)'
    				)->setParameter('id',array(403,404,405,410,411));
    	}
    	elseif ($area == "2" ) {
    		$query = $em->createQuery(
    				'SELECT n.id, n.nivel FROM SieAppWebBundle:NivelTipo n
                                    WHERE n.id IN (:id)'
    				)->setParameter('id',array(405,410,411));
    ...
    public function listarGradosAction($nivel) {
    	...
    	elseif ($nivel == "407"  ) {
    					$query = $em->createQuery(
    							'SELECT g.id, g.grado FROM SieAppWebBundle:GradoTipo g
                                    WHERE g.id IN (:id)'
    							)->setParameter('id',array(41,42,43));
    ...
    public function listarServiciosAction($area,$nivel,$grado) {
    	...
    	elseif (($area == "1" or $area == "2" or $area == "3" or $area == "4" or $area == "5" or $area == "8" or $area == "9")  and $nivel == "410" and  $grado == "99" ) {
    		$query = $em->createQuery(
    				'SELECT s.id, s.servicio FROM SieAppWebBundle:EspecialServicioTipo s
                                    WHERE s.id IN (:id)'
    				)->setParameter('id',array(1,2,3,4,5));
