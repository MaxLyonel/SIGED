...
class FuncionesController extends Controller {
    public function ObtenerUnidadEducativaAction($idUsuario,$gestion){
        ...
        $maestroInscripcion = $em->getRepository('SieAppWebBundle:MaestroInscripcion')->findOneBy(array('persona'=>$usuario->getPersona(),'gestionTipo'=>$gestion, 'esVigenteAdministrativo' => 't'));
        if(!$maestroInscripcion){
            return 0;
        }
        return $maestroInscripcion->getInstitucioneducativa()->getId();
    }
}