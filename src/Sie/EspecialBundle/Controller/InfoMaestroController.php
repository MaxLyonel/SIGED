...
//                ->getQuery();
    ...
    public function resultAction(Request $request) {
        ...
        $formVerificarPersona = $this->createForm(new VerificarPersonaSegipType(), 
            null,
            array(
                'action' => $this->generateUrl('herramienta_especial_info_maestro_segip_verificar_persona'),
                'method' => 'POST'
            )
        );
    ...
    public function verificarPersonaAction(Request $request){
        ...
        $formPersonaDatos = $this->createForm(new PersonaDatosType(), 
            null,
            array(
                'action' => $this->generateUrl('herramienta_especial_info_maestro_persona_datos'),
                'method' => 'POST'
            )
        );
    ...
    */
    ...
    private function newForm($idInstitucion, $gestion, $idPersona) {
        ...
        $form = $this->createFormBuilder()
                ...
                ->add('rda', 'text', array('label' => 'RDA', 'required' => false, 'attr' => array('autocomplete' => 'off', 'class' => 'form-control', 'pattern' => '[0-9]{1,10}')))
