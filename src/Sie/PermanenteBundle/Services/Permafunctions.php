<?php
namespace Sie\PermanenteBundle\Services;
...
// dump($arrDataInscription);die;
    $query = $this->em->getConnection()->prepare(" select * from departamento_tipo order by id ASC ");
    $query->execute();
    $objDeptos = $query->fetchAll();

    $arrDeptos = [];
    foreach ($objDeptos as $depto) {
      $arrDeptos[] = array(
      "id" => $depto['id'],
   ...
   */
   public function getOperatives(){
      ...
      $query = $this->em->getConnection()->prepare("
          select ost.id, ost.operativo 
          from sistema_tipo st
          left join operativo_sie_tipo ost on (ost.sistema_tipo_id = st.id)
          where st.id = 13 order by ost.id  ASC"
      );
      
      $query->execute();
      $operatives = $query->fetchAll();
