...
/**
 * InscriptionsCI controller.
 *
 */
class InscriptionsCIController extends Controller {
    ...
    public function indexAction(Request $request){
        ...
        return $this->render('SiePermanenteBundle:StudentsInscriptions:lookforstudentci.html.twig', array(
          'form'=>$this->findStudentciForm($infoUe)->createView()
        ));
    }
    /**
    * create the form to find the student by rude
    **/
    private function findStudentciForm($data){
      $form = $this->createFormBuilder()
              ->add('carnet','text', array('label'=>'carnet', 'attr'=> array('class'=>'form-control', 'placeholder' => 'Cedula', 'pattern' => '[A-Za-z0-9\sñÑ]{3,18}', 'maxlength' => '18', 'autocomplete' => 'off', 'style' => 'text-transform:uppercase')))
              ->add('data', 'hidden', array('data'=> $data))
              ->add('find', 'button', array('label'=> 'Buscar', 'attr'=>array('class'=>'btn btn-success', 'onclick'=>'findStudentci()')))
              ->getForm();
      return $form;
    }
    /**
    *find student method
    **/
    public function findStudentciAction(Request $request){
      ...
      $dataUe = unserialize($form['data']);
        ...
        $estadoServicio = $this->get('sie_app_web.segip')->estadoServicio();

        //$persona = $this->get('sie_app_web.persona')->buscarPersonaPorCarnetComplemento($form);
        $persona = null;
        if($estadoServicio){
            $persona = $this->get('sie_app_web.segip')->buscarPersonaPorCarnet($form['carnet']);
        }
        $infopersona= json_decode($persona['DatosPersonaEnFormatoJson']);
        dump($infopersona);die;
      ...
      $objStudent = $em->getRepository('SieAppWebBundle:Estudiante')->findOneBy(array('codigoRude'=>$form['cedula']));
    ...
    **/
    private function doInscriptionciForm($data, $studentId){
      $form = $this->createFormBuilder()
    ...
    **/
    public function saveInscriptionciAction(Request $request){
