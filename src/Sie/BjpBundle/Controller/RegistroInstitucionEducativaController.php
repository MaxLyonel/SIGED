...
class RegistroInstitucionEducativaController extends Controller {
    ...
    public function findieducativaAction(Request $request) {
        ...
        if (!$entities) {
            $this->get('session')->getFlashBag()->add('mensaje', 'No se encontro la información...');
...
// 		->add('id', 'text', array('label' => 'Codigo RUE','required' => true, 'pattern' => '[0-9]{8,17}', 'max_length' => '8'))
    ...
    public function createAction(Request $request) {
        try {
            ...
            $entity->setGestionTipo($em->getRepository('SieAppWebBundle:GestionTipo')->findOneById(2016));
            $entity->setLeJuridicciongeografica($em->getRepository('SieAppWebBundle:JurisdiccionGeografica')->findOneById($codigole));
...
//     		dump($areas);die;
    ...
    public function deleteAction(Request $request) {
        try {
            ...
            $gestion = $em->getRepository('SieAppWebBundle:GestionTipo')->findOneById(2016);

            if ($institucioneducativa) {
                $institucionNivelAutorizado = $em->getRepository('SieAppWebBundle:InstitucioneducativaNivelAutorizado')->findBy(array('gestionTipoId' => 2016, 'institucioneducativa' => $institucioneducativa->getId()));
