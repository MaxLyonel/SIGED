...
use Sie\AppWebBundle\Entity\BonojuancitoPaga;

/**
 * EstudianteInscripcion controller.
 *
 */
class PagosBonoController extends Controller {
    ...
    public function indexAction(Request $request){
      ...
      $query = $consultaAsig->createQueryBuilder('bjpa')
              ...
              ->andwhere('bjpum.gestionTipo = :gestion')
      ...
      {
        $sieArray[$value->getCodSie()] =$value->getInstitucioneducativa().'  '.$value->getCodSie();
        $contador = $contador + 1;
            ...
            $query1 = $consultaAsig1->createQueryBuilder('instedu')
              ...
              //    $contador = 0;
                  foreach ($resultAsig1 as $key => $value)
                  {
                    ...
                    $sieArray1[$value->getCodSie()] = $value->getInstitucioneducativa();
                    $contador = $contador + 1;
                    //echo '<script language="javascript">alert("'.$contador.'");</script>';
                  }
                  return $this->render('SieBjpBundle:PagosBono:index.html.twig', array(
             'form' => $this->seleccionUeForm($sieArray1)->createView()));

      }
      //dump($sieArray);die;
      else{return $this->render('SieBjpBundle:PagosBono:index.html.twig', array(
  ...
  public function buscarUeForm($datosUnidadEduAsignacion) {
    ...
    $datosCompleto = $datosUnidadEduAsignacion[0];
        ...
        return $this->createFormBuilder()
                    ->setAction($this->generateUrl('PagosBono_webForm'))
                    ...
                    ->add('aceptar', 'submit', array('label' => 'Aceptar', 'attr' => array('class' => 'btn btn-info btn')))
      ...
      //     ));
          return $this->render('SieBjpBundle:PagosBono:webform.html.twig', array(
                      // 'turnos' => $turnosArray, 'institucion' => $institucion, 'gestion' => $gestion,
                       ...
                       'dataForm'=> json_encode(array(
                         'sie'=>$formulario['sie'],'gestion'=>$this->session->get('currentyear')
                       ))
          ));
    }
        ...
        public function lista_areas_cursoAction(Request $request) {
                ...
                $estudianteInHabilitados = $em->getRepository('SieAppWebBundle:BonojuancitoAsignacion')->getEstudiantesBono($form, 'f','f');
                  ...
                  return $this->render('SieBjpBundle:PagosBono:estudiantesbono.html.twig', array(
                              ...
                              'formEstudiantesPagados' => $this->formWebPagados( json_encode($form))->createView(),
                               'data' => json_encode($form),
                              //  'formEstudianteForm' => $this->nuevoEStudianteForm(json_encode($form))->createView()

                  ));

        }
        private function nuevoEStudianteForm($data){
          $form = $this->createFormBuilder()
                  ->add('data', 'hidden', array('data' =>  $data))
                  ->add('paterno', 'text', array('label' => 'Paterno','required'=> false, 'attr' => array('class' => 'form-control')))
                  ->add('materno', 'text', array('label' => 'Materno','required'=> false, 'attr' => array('class' => 'form-control')))
                  ->add('nombre', 'text', array('label' => 'Nombre','required'=> false, 'attr' => array('class' => 'form-control')))
                  ->add('fnac', 'text', array('label' => 'Fecha Nacimiento','required'=> false, 'attr' => array('class' => 'form-control')))
                  ->add('rude', 'text', array('label' => 'Rude','required'=> false, 'attr' => array('class' => 'form-control')))
                  ->add('generoTipo', 'entity', array('class' => 'SieAppWebBundle:GeneroTipo', 'property' => 'genero', 'attr' => array('class' => 'form-control') ))
                  ->add('observacion', 'text', array('label' => 'Observación','required'=> false, 'attr' => array('class' => 'form-control')))
                  ->add('buscar', 'button', array('label' => 'Buscar', 'attr' => array('class' => 'btn btn-info btn-xs', 'onclick'=>'buscarNuevoEstudiante()')))
                  ->getForm();
          return $form;
        }
        
        private function nuevoCursoForm($data){
          $form = $this->createFormBuilder()
                  ->add('data', 'hidden', array('data' =>  $data))
                  ->add('paterno', 'text', array('label' => 'Paterno','required'=> false, 'attr' => array('class' => 'form-control')))
                  ->add('materno', 'text', array('label' => 'Materno','required'=> false, 'attr' => array('class' => 'form-control')))
                  ...
                  ->add('generoTipo', 'entity', array('class' => 'SieAppWebBundle:GeneroTipo', 'property' => 'genero', 'attr' => array('class' => 'form-control') ))
                  ->add('observacion', 'text', array('label' => 'Observación','required'=> false, 'attr' => array('class' => 'form-control')))
                  ->add('buscar', 'button', array('label' => 'Buscar', 'attr' => array('class' => 'btn btn-info btn-xs', 'onclick'=>'buscarNuevoCurso()')))
                  ->getForm();
          return $form;
        }

        public function nuevoEstudianteAction(Request $request){
          $data = $request->get('data');

          return $this->render('SieBjpBundle:PagosBono:nuevoEstudiante.html.twig', array(
                       'formEstudianteForm' => $this->nuevoEStudianteForm($data)->createView()
          ));
        }
        
        public function NuevoCursoBonoAction(Request $request){
          $data = $request->get('data');

          return $this->render('SieBjpBundle:PagosBono:nuevoCurso.html.twig', array(
                       'formCursoForm' => $this->nuevoCursoForm($data)->createView()
          ));
        }

        public function buscarNuevoEstudianteAction(Request $request){
          $form = $request->get('form');
          $busquedaEstudiante = $this->obtenerEstudianteNuevo($form);
          // dump($busquedaEstudiante);die;
          $dataEnviar = serialize($form);
          //  dump(($dataEnviar));
          //  dump(json_decode($dataEnviar));die;
          return $this->render('SieBjpBundle:PagosBono:estudianteDoble.html.twig', array(
                       'EstudianteResp' => $busquedaEstudiante['EstudianteResp'],
                       $busquedaEstudiante['respuesta'],
                       'dataEnviar'=> $dataEnviar
                     ));
          // dump($busquedaEstudiante);
          // die;
        }
        
        public function buscarNuevoEstudianteNuevoCursoAction(Request $request){
          $form = $request->get('form');
//          dump($form);
//          die;
          $busquedaEstudiante = $this->obtenerEstudianteNuevo($form);
          ...
          //  dump(($dataEnviar));
          //  dump(json_decode($dataEnviar));die;
          return $this->render('SieBjpBundle:PagosBono:estudianteDobleNuevoCurso.html.twig', array(
                       'EstudianteResp' => $busquedaEstudiante['EstudianteResp'],
                       $busquedaEstudiante['respuesta'],
                       'dataEnviar'=> $dataEnviar
                     ));
        ...
        private function obtenerEstudianteNuevo($data){
          $em = $this->getDoctrine()->getManager();
          $respuesta = false;

          $datoInscripcion = $em->getRepository('SieAppWebBundle:BonojuancitoPaga');

          if($data['rude']){
            $query = $datoInscripcion->createQueryBuilder('bjp')
                    ->select('bjp')
                    ->where('bjp.codigoRude = :rude')
                    ->setParameter('rude', $data['rude'])
                    ->getQuery();
          }else{
            $query = $datoInscripcion->createQueryBuilder('bjp')
            ->where('bjp.paterno = :paterno')
            ->andwhere('bjp.materno = :materno')
            ->andwhere('bjp.nombre = :nombre')
            ...
            // ->setParameter('fnac',$data['fnac'])
            ->getQuery();
          }

          $estudianteBono = $query->getResult();

          // $estudianteBonoArray = array();
          if($estudianteBono){
            $respuesta = 'bono';
          //   $estudianteBonoArray = array(
          //     'id'=> $estudianteBono[0]->getId(),
          //     'paterno'=> $estudianteBono[0]->getPaterno(),
          //     'materno'=> $estudianteBono[0]->getMaterno(),
          //     'nombre'=> $estudianteBono[0]->getNombre(),
          //     'codigoRude'=> $estudianteBono[0]->getCodigoRude(),
          //     'fechaNacimiento'=> $estudianteBono[0]->getfechaNacimiento(),
          //   );
          }else{

            $datoInscripcion = $em->getRepository('SieAppWebBundle:Estudiante');

            if($data['rude']){
              $query = $datoInscripcion->createQueryBuilder('est')
                      // ->select('est')
                      ->where('est.codigoRude = :rude')
                      ->setParameter('rude', $data['rude'])
                      ->getQuery();
            }else{
              $query = $datoInscripcion->createQueryBuilder('est')
              ->where('est.paterno = :paterno')
              ->andwhere('est.materno = :materno')
              ->andwhere('est.nombre = :nombre')
              // ->andwhere('est.fechaNacimiento = :fnac')
              ->setParameter('paterno', mb_strtoupper($data['paterno'], 'utf8'))
              ->setParameter('materno', mb_strtoupper($data['materno'], 'utf8'))
              ->setParameter('nombre', mb_strtoupper($data['nombre'], 'utf8'))
              // ->setParameter('fnac',$data['fnac'])
          ...
          return array('respuesta'=>$respuesta,'EstudianteResp'=>$estudianteBono);

        }

        public function inscribirBonoCursoAction(Request $request){
          $form = $request->get('form');          
          ...
          $dataEnviado = unserialize($request->get('dataEnviar'));
          $idEstudiante = $request->get('idEstudiante');
          //$paralelo = $request->get('paralelo');          
          $sesion = $request->getSession();          
          $datcurso = $sesion->get('bjpdatcurso'); 
          $bjpumid = $sesion->get('bjpumid');
          //$bjpumid = $sesion->get('bjpumid');
          
//          dump($paralelo);
//          dump($datcurso['paralelo']);
//          die;
          $par = $datcurso['paralelo'];
          
          
          $query = "  select b.id as bid, b.estadomatricula_tipo_id as estadomatricula_tipo_id, a.* from estudiante a
                        inner join estudiante_inscripcion b on b.estudiante_id = a.id
                        where a.id = '".$idEstudiante."'
                        order by b.id  DESC";

          $datalumnos = $db->prepare($query);
          $params = array();
          $datalumnos->execute($params);
          $datalumno = $datalumnos->fetchAll();
...
//          dump($idEstudiante);          
          ...
          $query = $em->getConnection()->prepare("select * from sp_reinicia_secuencia('bonojuancito_paga');");
          $query->execute();
          $bjp = new BonojuancitoPaga();
          $bjp->setGestionTipo($em->getRepository('SieAppWebBundle:GestionTipo')->find('2016'));
          $bjp->setBonojuancitoUnidadmilitar($em->getRepository('SieAppWebBundle:BonojuancitoUnidadmilitar')->find($bjpumid)); 
          $bjp->setEstudianteInscripcion($em->getRepository('SieAppWebBundle:EstudianteInscripcion')->find($idEstudiante));
          $bjp->setCodigoRude($dataEnviado['rude']);
          $bjp->setInstitucioneducativa($em->getRepository('SieAppWebBundle:Institucioneducativa')->find($datcurso['idInstitucion']));
          $bjp->setCarnetIdentidad($datalumno[0]['carnet_identidad']);
          $bjp->setPaterno($datalumno[0]['paterno']);
          $bjp->setMaterno($datalumno[0]['materno']);
          $bjp->setNombre($datalumno[0]['nombre']);
          $bjp->setFechaNacimiento(new \DateTime($datalumno[0]['fecha_nacimiento']));
          $bjp->setGenero($datalumno[0]['genero_tipo_id']);
          $bjp->setNivelTipo($em->getRepository('SieAppWebBundle:NivelTipo')->find($datcurso['nivel']));
          $bjp->setNivel($em->getRepository('SieAppWebBundle:NivelTipo')->find($datcurso['nivel'])->getNivel());
          $bjp->setGradoTipo($em->getRepository('SieAppWebBundle:GradoTipo')->find($datcurso['grado']));
          $bjp->setGrado($em->getRepository('SieAppWebBundle:GradoTipo')->find($datcurso['grado'])->getGrado());
          $bjp->setParaleloTipo($em->getRepository('SieAppWebBundle:ParaleloTipo')->find($par));
          $bjp->setParalelo($em->getRepository('SieAppWebBundle:ParaleloTipo')->find($par)->getParalelo());
          $bjp->setIdTurno($em->getRepository('SieAppWebBundle:TurnoTipo')->find('1'));
          $bjp->setTurno($em->getRepository('SieAppWebBundle:TurnoTipo')->find('1')->getTurno());
          $bjp->setEstadomatriculaTipoId($datalumno[0]['estadomatricula_tipo_id']);
          $bjp->setEstadomatriculaTipo($em->getRepository('SieAppWebBundle:EstadomatriculaTipo')->find($datalumno[0]['estadomatricula_tipo_id']));
          $bjp->setEsnuevoingreso(true);
          $bjp->setEspagado(false);
          $bjp->setEshabilitado(true);
          //$bjp->setLugar($lugar);
          $bjp->setEsnuevorude(false);
          //$bjp->setObservaciones($observaciones);
          //$bjp->setEstadopagoEstudiante($estadopagoEstudiante);
          $bjp->setEstudiante($em->getRepository('SieAppWebBundle:Estudiante')->find($idEstudiante));
          
          $em->persist($bjp);
          $em->flush();
            
          //$data = json_decode($dataEnviado['data'], true);

          //dump($request);die;
          
          //$form = $data;
          
          // $this->actualizarEStadoEStudiante($form,'habilitados2');
          // dump($form);die;
          // $data = json_decode($form['data'],true);
          $estudianteHabilitados = $em->getRepository('SieAppWebBundle:BonojuancitoAsignacion')->getEstudiantesBono($datcurso, 'f','t');
          $estudiantePagados = $em->getRepository('SieAppWebBundle:BonojuancitoAsignacion')->getEstudiantesBono($datcurso, 't','t');
          $estudianteInHabilitados = $em->getRepository('SieAppWebBundle:BonojuancitoAsignacion')->getEstudiantesBono($datcurso, 'f','f');
            ...
            return $this->render('SieBjpBundle:PagosBono:estudiantesbono.html.twig', array(
                        ...
                        'formEstudiantesPagados' => $this->formWebPagados( json_encode($data))->createView(),
                        'data' => json_encode($data)

            ));
        }

        public function inscribirBonoCursoNuevoParaleloAction(Request $request){
          ...
          $bjp->setFechaNacimiento(new \DateTime($datalumno[0]['fecha_nacimiento']));
          $bjp->setGenero($datalumno[0]['genero_tipo_id']);
          $bjp->setNivelTipo($em->getRepository('SieAppWebBundle:NivelTipo')->find($nivel));
          $bjp->setNivel($em->getRepository('SieAppWebBundle:NivelTipo')->find($nivel)->getNivel());
          $bjp->setGradoTipo($em->getRepository('SieAppWebBundle:GradoTipo')->find($grado));
          $bjp->setGrado($em->getRepository('SieAppWebBundle:GradoTipo')->find($grado)->getGrado());
          $bjp->setParaleloTipo($em->getRepository('SieAppWebBundle:ParaleloTipo')->find($par));
          $bjp->setParalelo($em->getRepository('SieAppWebBundle:ParaleloTipo')->find($par)->getParalelo());
          $bjp->setIdTurno($em->getRepository('SieAppWebBundle:TurnoTipo')->find('1'));
          $bjp->setTurno($em->getRepository('SieAppWebBundle:TurnoTipo')->find('1')->getTurno());
        ...
        public function inscribirBonoNuevoCursoAction(Request $request){
          $em = $this->getDoctrine()->getManager();
          
          $dataEnviado = $request->get('form'); 
          //$paralelo = $request->get('paralelo');  
          $idEstudiante = '999';          
          $idMatricula = '0'; 
          $carnet = '';
          ...
          $datcurso = $sesion->get('bjpdatcurso'); 
          $bjpumid = $sesion->get('bjpumid');          
          $porciones = explode("-", $dataEnviado['fnac']);
          $fechnac = $porciones[2].'-'.$porciones[1].'-'.$porciones[0];
          
          $par = $datcurso['paralelo'];
          
          $digits = 4;
          $mat = str_pad(rand(0, pow(10, $digits) - 1), $digits, '0', STR_PAD_LEFT);
          $rude = 'F'.$datcurso['idInstitucion'] . $this->session->get('currentyear') . $mat;
                    
          //$rude = 'F'.$datcurso['idInstitucion'].$datcurso['idGestion'];
          $rudedup = $em->getRepository('SieAppWebBundle:Estudiante')->findByCodigoRude($rude);
          if ($rudedup){
              $rude = $rude.'D';
          }
          $query = $em->getConnection()->prepare("select * from sp_reinicia_secuencia('bonojuancito_paga');");
          ...
          $bjp->setBonojuancitoUnidadmilitar($em->getRepository('SieAppWebBundle:BonojuancitoUnidadmilitar')->find($bjpumid)); 
          $bjp->setEstudianteInscripcion($em->getRepository('SieAppWebBundle:EstudianteInscripcion')->find($idEstudiante));
          $bjp->setCodigoRude($rude);
          $bjp->setInstitucioneducativa($em->getRepository('SieAppWebBundle:Institucioneducativa')->find($datcurso['idInstitucion']));
          $bjp->setCarnetIdentidad($carnet);
          $bjp->setPaterno($dataEnviado['paterno']);
          $bjp->setMaterno($dataEnviado['materno']);
          $bjp->setNombre($dataEnviado['nombre']);
          $bjp->setFechaNacimiento(new \DateTime($fechnac));
          $bjp->setGenero($dataEnviado['generoTipo']);
          $bjp->setNivelTipo($em->getRepository('SieAppWebBundle:NivelTipo')->find($datcurso['nivel']));
          $bjp->setNivel($em->getRepository('SieAppWebBundle:NivelTipo')->find($datcurso['nivel'])->getNivel());
          $bjp->setGradoTipo($em->getRepository('SieAppWebBundle:GradoTipo')->find($datcurso['grado']));
          $bjp->setGrado($em->getRepository('SieAppWebBundle:GradoTipo')->find($datcurso['grado'])->getGrado());
          ...
          $bjp->setIdTurno($em->getRepository('SieAppWebBundle:TurnoTipo')->find('1'));
          $bjp->setTurno($em->getRepository('SieAppWebBundle:TurnoTipo')->find('1')->getTurno());
          $bjp->setEstadomatriculaTipoId($idMatricula);
          $bjp->setEstadomatriculaTipo($em->getRepository('SieAppWebBundle:EstadomatriculaTipo')->find($idMatricula));
          $bjp->setEsnuevoingreso(true);
          $bjp->setEspagado(false);
          $bjp->setEshabilitado(true);
          //$bjp->setLugar($lugar);
          $bjp->setEsnuevorude(true);
          //$bjp->setObservaciones($observaciones);
          //$bjp->setEstadopagoEstudiante($estadopagoEstudiante);
          $bjp->setEstudiante($em->getRepository('SieAppWebBundle:Estudiante')->find($idEstudiante));
          ...
          $estudianteInHabilitados = $em->getRepository('SieAppWebBundle:BonojuancitoAsignacion')->getEstudiantesBono($datcurso, 'f','f');
            ...
            return $this->render('SieBjpBundle:PagosBono:estudiantesbono.html.twig', array(
                        ...
                        'estudiantePagados' => $estudiantePagados,
                        'estudianteInHabilitados' => $estudianteInHabilitados,
                        'formEstudiantes' => $this->formWebForm( json_encode($dataEnviado))->createView(),
                        'formEstudiantesPagados' => $this->formWebPagados( json_encode($dataEnviado))->createView(),
                        'data' => json_encode($dataEnviado)

            ));
        }
        
        public function inscribirBonoNuevoCursoNuevoParaleloAction(Request $request){
          $em = $this->getDoctrine()->getManager();
...
//          die;
        ...
        public function eliminarBonoNuevoCursoAction(Request $request){
    
          $em = $this->getDoctrine()->getManager();
          $sesion = $request->getSession(); 
          $datcurso = $sesion->get('bjpdatcurso'); 
          $dataEnviado = $request->get('data'); 
          $dataEnviadoaux = substr($dataEnviado, 0, -1);
          $porciones = explode("|", $dataEnviadoaux);
          //dump($datcurso);
          //¿die;
          foreach ($porciones as $id) {                    
                    $bjp = $em->getRepository('SieAppWebBundle:BonojuancitoPaga')->find($id);          
                    if ($bjp->getEsnuevorude() == 'true'){
                        $em->remove($bjp);
                        $em->flush();                        
                    }    
//                    }else{
//                        $bjp->setEshabilitado(false);
//                        $em->persist($bjp);
//                        $em->flush(); 
//                    }
...
// dump($form);die;
          switch ($form['movida']) {
            case '1':
              $this->actualizarEStadoEStudiante($form,'pagadosentiempo');
              break;
            case '2':
              $this->actualizarEStadoEStudiante($form,'pagadosrezagados');
              break;
            case '3':
              $this->actualizarEStadoEStudiante($form,'inhabilitadosabandono');
              break;
            case '4':
              $this->actualizarEStadoEStudiante($form,'inhabilitadostraspaso');
              break;
            case '5':
              $this->actualizarEStadoEStudiante($form,'inhabilitadosotro');
              break;
        ...
        private function actualizarEStadoEStudiante($form,$estado){
          ...
          foreach ($form as $key => $value) {
            ...
            if($estudianteMover=='habmov'){
              ...
              switch ($estado) {
                case 'pagadosentiempo':
                  # code...
                  $estudianteModificar->setEspagado('t');
                  $estudianteModificar->setEstadopagoEstudiante('ET');
                  break;
                case 'pagadosrezagados':
                    # code...
                  $estudianteModificar->setEspagado('t');
                  $estudianteModificar->setEstadopagoEstudiante('R');
                  break;
                case 'inhabilitadosabandono':
                  # code...
                  $estudianteModificar->setEshabilitado('f');
                  $estudianteModificar->setEstadopagoEstudiante('A');
                  break;
                case 'inhabilitadostraspaso':
                  # code...
                  $estudianteModificar->setEshabilitado('f');
                  $estudianteModificar->setEstadopagoEstudiante('T');
                  break;
                case 'inhabilitadosotro':
                  # code...
                  $estudianteModificar->setEshabilitado('f');
                  $estudianteModificar->setEstadopagoEstudiante('O');
                  break;
                case 'habilitados':
                  # code...
                  $estudianteModificar->setEspagado('f');
                  $estudianteModificar->setEstadopagoEstudiante(null);
                  break;
                case 'habilitados2':
                  # code...
                  $estudianteModificar->setEshabilitado('t');
                  $estudianteModificar->setEstadopagoEstudiante(null);
                  break;
        ...
        private  function formWebPagados($data){

          $form = $this->createFormBuilder()
                  ->add('data', 'text', array('data' =>  $data))
                  ->add('datakrlos', 'text', array('data' =>  'krlos'))
                  // ->add('movida', 'choice', array('label' => ' ','required'=> false, 'empty_value' => 'Seleccionar...', 'choices' => array(), 'attr' => array('class' => 'form-control')))
                  ->add('moverPagados', 'button', array('label' => 'Mover', 'attr' => array('class' => 'btn btn-warning btn-block', 'onclick'=>'moverEstPagados()')))
                  ->getForm();
          return $form;
        }

        public function moverPagadosAction(Request $request){
          // dump($request);die;
          $form = $request->get('form');
          $em = $this->getDoctrine()->getManager();
          $this->actualizarEStadoEStudiante($form,'habilitados');
// dump($form);die;
          $data = json_decode($form['data'],true);
          $estudianteHabilitados = $em->getRepository('SieAppWebBundle:BonojuancitoAsignacion')->getEstudiantesBono($data, 'f','t');
          $estudiantePagados = $em->getRepository('SieAppWebBundle:BonojuancitoAsignacion')->getEstudiantesBono($data, 't','t');
        ...
        public function moverInhabilitadosAction(Request $request){
          // dump($request);die;
          $form = $request->get('form');
          $em = $this->getDoctrine()->getManager();
          $this->actualizarEStadoEStudiante($form,'habilitados2');
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        try {
            ...
            switch ($tipoSistema) {
                ...
                case 'ALTERNATIVA': $this->session->set('tituloTipo', 'Adición de Áreas y Asignacion de Docentes');
                    $this->session->set('layout', 'layoutBjp.html.twig');
                    break;
                default: $this->session->set('tituloTipo', 'PagosBono');
                    $this->session->set('layout', 'layoutBjp.html.twig');
                    break;
            }
            ...
            if ($request->getMethod() == 'POST') { // si los valores fueron enviados desde el formulario
                ...
                if ($form['gestion'] < 2008 or $form['gestion'] > 2017) {
                    ...
                    return $this->render('SieBjpBundle:PagosBono:search.html.twig', array('form' => $this->formSearch($request->getSession()->get('currentyear'))->createView()));
                }
                /**
    ...
    private function formSearch($gestionactual) {
        ...
        $form = $this->createFormBuilder()
                ->setAction($this->generateUrl('PagosBono'))
