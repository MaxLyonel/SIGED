 ...
 *
 */
class EstudianteNotasPrimariaController extends Controller {
    ...
    public function createUpdateAction(Request $request){
        ...
        for($i=0;$i<count($idEstudianteNotaCualitativa);$i++) {
            ...
            if($idEstudianteNotaCualitativa[$i] == 'nuevo'){
                ...
                $newNotaCualitativa->setEstudianteInscripcion($inscripcion);
                $newNotaCualitativa->setNotaCuantitativa(0);
                $newNotaCualitativa->setNotaCualitativa($notaCualitativa[$i]);
                ...
                $updateNotaCualitativa = $em->getRepository('SieAppWebBundle:EstudianteNotaCualitativa')->find($idEstudianteNotaCualitativa[$i]);
                if($updateNotaCualitativa){
                    $updateNotaCualitativa->setNotaCualitativa(mb_strtoupper($notaCualitativa[$i],'utf-8'));
                    $updateNotaCualitativa->setUsuarioId($this->session->get('userId'));
                    $updateNotaCualitativa->setFechaModificacion(new \DateTime('now'));
                    $em->flush();
                }
            }
        }
    ...
    public function actualizarEstadoMatricula(){
