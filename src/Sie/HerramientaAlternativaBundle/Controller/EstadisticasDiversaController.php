...
class EstadisticasDiversaController extends Controller
{
    private $session;
...
//        die;
    ...
    public function diversaIndexAction(Request $request) {
//dump($request);die;
...
//                }
      ...
      //  dump($subEntidades);die;
        ...
        $chartDiversa= $this->chartDonutInformacionGeneralNivelGrado($entityEstadistica[1],"Estudiantes matriculados según Educación Diversa",$gestion,"Estudiantes","chartContainerDiversa");
//        //$chartNivelGrado = $this->chartDonutInformacionGeneralNivelGrado($entityEstadistica,"Estudiantes Matriculados según Nivel de Estudio y Año de Escolaridad ",$gestionProcesada,6,"chartContainerEfectivoNivelGrado");
        $chartGenero= $this->chartPie($entityEstadistica[2],"Estudiantes matriculados según Sexo",$gestion,"Estudiantes","chartContainerGenero");
//        //$chartArea = $this->chartPyramidInformacionGeneral($entityEstadistica,"Estudiantes Matriculados según Área Geográfica",$gestionProcesada,4,"chartContainerEfectivoArea");
  //      $chartDependencia = $this->chartColumn($entityEstadistica[3],"Estudiantes matriculados según Unidades Militares",$gestion,"Estudiantes","chartContainerDependencia");
...
//        $chartModalidad = $this->chartSemiPieDonut3d($entityEstadistica[4],"Estudiantes matriculados según Modalidad",$gestion,"Estudiantes","chartContainerModalidad");
        ...
        if(count($subEntidades)>0 and isset($subEntidades)){
            return $this->render($this->session->get('pathSystem') . ':Reporte:matriculaEducativaDiversa.html.twig', array(
                'infoEntidad'=>$entidad,
                'infoSubEntidad'=>$subEntidades,
                'gestion'=>$gestion,
                'datoGraficoNivel'=>$chartNivel,
                'datoGraficoDiversa'=>$chartDiversa,
    ...
    public function buscaEstadisticaAlternativaAreaRol($area,$rol,$gestion,$periodo) {
      ...
      // $gestionProcesada = $this->buscaGestionVistaMaterializadaAlternativa();
        ...
        with tabla as (
            SELECT ie.dependencia_tipo_id as dependencia_id, a.id as area_id,b.id as especialidad_id,d.id as acreditacion_id,es.genero_tipo_id as genero_id,q.id as diversa_id
            ,zz.id as fuerza_militar, yy.id as lugar_reclusion
            --,case q.id when 1 then 0 when 2 then zz.id when 3 then yy.id when 4 then 0 else 0 end as recinto_id
                       , count(*) as cantidad
                        from superior_facultad_area_tipo a  
                       inner join superior_especialidad_tipo b on a.id=b.superior_facultad_area_tipo_id 
                       inner join superior_acreditacion_especialidad c on b.id=c.superior_especialidad_tipo_id 
                       ...
                       inner join estudiante as es on es.id = i.estudiante_id
                                   inner join rude ru on ru.estudiante_inscripcion_id = i.id 
                                   inner join rude_educacion_diversa red on red.rude_id =ru.id
                                   inner join educacion_diversa_tipo q on red.educacion_diversa_tipo_id =q.id and q.id in (2,3,4)
                                   inner join unidad_militar_tipo r on COALESCE( red.unidad_militar_tipo_id,0)=r.id 
                       inner join recinto_penitenciario_tipo s on COALESCE(red.recinto_penitenciario_tipo_id,0)=s.id 
                                   inner join fuerza_militar_tipo zz on r.fuerza_militar_tipo_id=zz.id
                       inner join lugar_reclusion_tipo yy on lugar_reclusion_tipo_id=yy.id
                       ...
                       group by ie.dependencia_tipo_id,a.id,b.id,d.id,es.genero_tipo_id,q.id,zz.id,yy.id
           )
           select 1 as tipo_id,'Educacion Diversa' as tipo_nombre
           , case 
																							 when ed.id =2 and b.zz = '0' then 0
                                               when ed.id =2 and b.zz = '1' then 1
                                               when ed.id =2 and b.zz = '2' then 2
                                               when ed.id =2 and b.zz = '3' then 3
                                               when ed.id =3 and b.yy = '0' then 4
                                               when ed.id =3 and b.yy = '1' then 5
                                               when ed.id =3 and b.yy = '2' then 6
                                               when ed.id =3 and b.yy = '3' then 7
                                               when ed.id =4 then 8
                                               else 0
                                           end as id
				 	 , case 
											   when ed.id =2 and b.zz = '0' then 'UM'
                                               when ed.id =2 and b.zz = '1' then 'UM'
                                               when ed.id =2 and b.zz = '2' then 'UM'
                                               when ed.id =2 and b.zz = '3' then 'UM'
                                               when ed.id =3 and b.yy = '0' then 'RP'
                                               when ed.id =3 and b.yy = '1' then 'RP'
                                               when ed.id =3 and b.yy = '2' then 'RP'
                                               when ed.id =3 and b.yy = '3' then 'RP'
                                               when ed.id =4 then 'TH'
                                               else ''
                                       end  as nombre			 
	
					 ,replace(b.recinto,'NINGUNO','SIN ESPECIFICAR') as subnombre,sum( cantidad ) as cantidad
            from (
           select zz.id as zz,yy.id as yy,dependencia_id, area_id,especialidad_id,acreditacion_id,genero_id,diversa_id
					 ,case diversa_id when 2 then t.fuerza_militar 
													  when 3 then t.lugar_reclusion else 0 
													  end as recinto_id
					 ,case diversa_id when 2 then zz.fuerza_militar 
														when 3 then yy.lugar_reclusion else 'SIN ESPECIFICAR' 
														end as recinto,cantidad 
														from tabla as t 
           left join fuerza_militar_tipo zz on zz.id=t.fuerza_militar 
           left join lugar_reclusion_tipo yy on yy.id=t.lugar_reclusion 
           ) as b
           inner join educacion_diversa_tipo as ed on ed.id = b.diversa_id
           group by b.zz,b.yy,ed.id,ed.educacion_diversa,b.recinto_id,b.recinto
             union all
                 select 2 as tipo_id, 'Sexo' as tipo_nombre, gt.id, gt.genero as nombre,'' as subnombre
        ...
        if($rol == 10 or $rol == 11) // Distrital o Tecnico Distrito
        {
            ...
            with tabla as (
               ...
               group by b.zz,b.yy,ed.id,ed.educacion_diversa,b.recinto_id,b.recinto
                 union all
                     select 2 as tipo_id, 'Sexo' as tipo_nombre, gt.id, gt.genero as nombre, ed.educacion_diversa as subnombre
                               , sum(cantidad) as cantidad from tabla as t 
                               inner join genero_tipo as gt on gt.id = t.genero_id
                                           inner join educacion_diversa_tipo as ed on ed.id = t.diversa_id and ed.id in (2,3,4)
                               group by gt.id, gt.genero,ed.educacion_diversa     
               union all
