...
class EstudianteController extends Controller {
    ...
    private function createSearchForm() {
        ...
        $form = $this->createFormBuilder($estudiante)
                ->setAction($this->generateUrl('student_main_result'))
                ...
                ->add('lookfor', 'submit', array('label' => 'Buscar', 'attr' => array('class' => 'btn btn-default')))
                ->getForm();
        return $form;
    }
    ...
    public function searchstudentAction(Request $request) {
        ...
        $query = $repository->createQueryBuilder('e')
                ...
                ->setParameter('paterno', strtoupper($form['paterno']) . '%')
                ->setParameter('materno', strtoupper($form['materno']) . '%')
                ->setParameter('nombre', strtoupper($form['nombre']) . '%')
                ->orderBy('e.paterno, e.materno, e.nombre', 'ASC')
                ->getQuery();
        $entities = $query->getResult();
        if (!$entities) {
            return $this->render($this->session->get('pathSystem') . ':Estudiante:searchstudent.html.twig', array(
                        'form' => $this->createSearchForm()->createView(), 'nodata' => 'Resultado No encontrado'
            ));
        }
    ...
    private function createNewForm() {
        ...
        $form = $this->createFormBuilder($estudiante)
                ...
                ->add('lugarProvNacTipo', 'entity', array('class' => 'SieAppWebBundle:LugarTipo',
    ...
    public function createAction(Request $request) {
        try {
            ...
            $estudiante->setLugarProvNacTipo($em->getRepository('SieAppWebBundle:LugarTipo')->findOneById($form['lugarNacTipo']));
