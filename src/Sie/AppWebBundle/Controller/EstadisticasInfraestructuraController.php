...
class EstadisticasInfraestructuraController extends Controller
{
    private $session;
    ...
    public function indexAction(){
        ...
        return $this->render('SieAppWebBundle:Default:index.html.twig');
    }
...
//        die;
    ...
    public function infraestructuraIndexAction(Request $request) {
//dump($request);die;
...
//                }
                $totalgeneral=$totalgeneral+ $dato['total_ambiente'];
            }
            foreach ($subEntidades as $key => $dato) {
//                if(isset(reset($entityEstadistica)['dato'][0]['cantidad'])){
//                    $subEntidades[$key]['total_general'] = reset($entityEstadistica)['dato'][0]['cantidad'];
...
//                }
      ...
      //  dump($subEntidades);die;
        ...
        $chartPoblacion = $this->chartColumn($entityEstadistica[1],"Cantidades según el tipo de Ambiente",$gestion,"Ambientes","chartContainerPoblacion");
...
//        //$chartNivelGrado = $this->chartDonutInformacionGeneralNivelGrado($entityEstadistica,"Estudiantes Matriculados según Nivel de Estudio y Año de Escolaridad ",$gestionProcesada,6,"chartContainerEfectivoNivelGrado");
        $chartGenero= $this->chartPie($entityEstadistica[2],"Cantidades según el tipo de Predio",$gestion,"Ambientes","chartContainerGenero");
//        //$chartArea = $this->chartPyramidInformacionGeneral($entityEstadistica,"Estudiantes Matriculados según Área Geográfica",$gestionProcesada,4,"chartContainerEfectivoArea");
        $chartDependencia = $this->chartSemiPieDonut3d($entityEstadistica[3],"Cantidades según el Servicio Electrico",$gestion,"Ambientes","chartContainerDependencia");
...
//dump($chartDependencia);die;
     ...
     * @return type
     */
    public function buscaEstadisticaInfraestructuraAreaRol($area,$rol,$gestion,$periodo) {
        /*
         * Define la zona horaria y halla la fecha actual
         */
        date_default_timezone_set('America/La_Paz');
      ...
      // $gestionProcesada = $this->buscaGestionVistaMaterializadaAlternativa();
        ...
        with tabla as (												 
            SELECT ia.id,ip.infra_predio_tipo_id as prediotipoid,iact.id as ambientetipoid
            ,count(*) as cantidad														 
            ...
            inner join infra_predio ip on ip.id =ia.infra_predio_id 
            inner join infra_predio_tipo ipt on ipt.id = ip.infra_predio_tipo_id 
            inner join jurisdiccion_geografica jg on jg.id = ip.jurisdiccion_geografica_id
            ...
            where ip.gestion_tipo_id =  ".$gestionActual." --and jg.id='".$area."'
            group by ia.id, ip.infra_predio_tipo_id,iact.id									 
            )								
            select 1 as tipo_id, 'Ambiente Tipo' as tipo_nombre, iact.id, iact.ambiente_categoria as nombre, '' as subnombre
            , sum(cantidad) as cantidad from tabla as t 
            inner join infra_ambiente_categoria_tipo as iact on iact.id = t.ambientetipoid
            group by iact.id, iact.ambiente_categoria		 
            union all
            select 2 as tipo_id, 'Predio Tipo' as tipo_nombre, ipt.id, ipt.nivel as nombre, '' as subnombre
            , sum(cantidad) as cantidad from tabla as t 
            inner join infra_predio_tipo as ipt on ipt.id = t.prediotipoid
            group by ipt.id, ipt.nivel		
            union all
            select 3 as tipo_id, 'Cuenta con Servicio Electrico' as tipo_nombre
            ,case when ia.servicio_electrico = true then 1
                        when ia.servicio_electrico = false then 2 else  0	end as id
            ,case when ia.servicio_electrico = true then 'Con servicio Electrico'
                        when ia.servicio_electrico = false then 'Sin servicio Electrico' else  ''	end as nombre, '' as subnombre
            , sum(t.cantidad) as cantidad from tabla as t 
            inner join infra_ambiente as ia on ia.id = t.id
            group by ia.servicio_electrico
        ...
        if($rol == 10 or $rol == 11) // Distrital o Tecnico Distrito
        {
            ...
            with tabla as (												 
                ...
                left join lugar_tipo as lt4 on lt4.id = lt3.lugar_tipo_id
                left join lugar_tipo as lt5 on lt5.id = jg.lugar_tipo_id_distrito
                where ip.gestion_tipo_id =  ".$gestionActual." and lt5.codigo='".$area."'
                group by ia.id,ip.infra_predio_tipo_id,iact.id									 
                )								
                select 1 as tipo_id, 'Ambiente Tipo' as tipo_nombre, iact.id, iact.ambiente_categoria as nombre, '' as subnombre
                , sum(cantidad) as cantidad from tabla as t 
        ...
        if($rol == 7) // Tecnico Departamental
        {
            ...
            with tabla as (												 
                ...
                left join lugar_tipo as lt4 on lt4.id = lt3.lugar_tipo_id
                left join lugar_tipo as lt5 on lt5.id = jg.lugar_tipo_id_distrito
                where ip.gestion_tipo_id =  ".$gestionActual." and lt4.codigo='".$area."'
                group by ia.id,ip.infra_predio_tipo_id,iact.id									 
                )								
                select 1 as tipo_id, 'Ambiente Tipo' as tipo_nombre, iact.id, iact.ambiente_categoria as nombre, '' as subnombre
                , sum(cantidad) as cantidad from tabla as t 
   ...
   from infra_ambiente ia
    ...
    left join lugar_tipo as lt4 on lt4.id = lt3.lugar_tipo_id
    left join lugar_tipo as lt5 on lt5.id = jg.lugar_tipo_id_distrito
    where ip.gestion_tipo_id = ".$gestionActual."
    group by lt4.id, lt4.codigo, lt4.lugar															 
        ...
        {
            $queryEntidad = $em->getConnection()->prepare("
            SELECT 'Edificio Educativo' as nombreArea, jg.id as codigo, jg.id as nombre, 9 as rolUsuario 
            , count(*) as total_ambiente  
         from infra_ambiente ia
         inner join infra_ambiente_tipo iat on iat.id = ia.infra_ambiente_tipo_id
         inner join infra_ambiente_categoria_tipo iact on iact.id = iat.infra_ambiente_categoria_tipo_id
         ...
         left join lugar_tipo as lt5 on lt5.id = jg.lugar_tipo_id_distrito
         where iP.gestion_tipo_id = ".$gestionActual." and lt5.codigo='".$area."'
         group by jg.id, jg.id
        order by jg.id, jg.id
                ");
        }
