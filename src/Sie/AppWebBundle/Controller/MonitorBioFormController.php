...
class MonitorBioFormController extends Controller{
 ...
 */	
    public function indexAction(Request $request){
        ...
        return $this->render($this->session->get('pathSystem').':MonitorBioForm:index.html.twig', array(
            'rol' => $userRol,
   ...
   private function getUnidadesEducativas($departamento=-1,$distrito=-1,$gestion){
        ...
        if(!$tmp){
        	...
        	foreach ($tmp as $value) {
        		...
        		if($objBioInstitucioneducativaBioseguridad){
        			...
        			$cc=0;
	        		foreach ($objBioInstitucioneducativaBioseguridad as $val) {
	        			
	        			$arrformbio[] = array('week'=>$val->getSemana(), 'month'=>$this->month[$val->getMes()]);
	        			$cc++;
	        			if($cc==3)break;
        		...
        		$ues[] = array('id'=>$value['id'],'institucioneducativa'=>$value['institucioneducativa'], 'regformbio'=>$arrformbio);
        		// $ues[]=arary('id'=>$value['id'],'institucioneducativa'=>$value['institucioneducativa'], 'regformbio'=>$arrformbio);
    ...
    public function index1Action(){
        ...
        return $this->render('SieAppWebBundle:MonitorBioForm:index.html.twig', array(
                // ...
            ));    
    }
