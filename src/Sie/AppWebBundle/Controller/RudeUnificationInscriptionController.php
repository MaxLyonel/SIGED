...
use Sie\AppWebBundle\Entity\ApoderadoInscripcion;
 ...
 /**
     * Modulo para la Unificación de Rudes para las UE
    */
class RudeUnificationInscriptionController extends Controller{
    ...
    public function indexAction(Request $request){
        ...
        return $this->render('SieAppWebBundle:RudeUnificationInscription:index.html.twig', array(
                'rudeStudent' => $arrStudent
            ));    
    }
    
    public function lookforStudentsHistoryAction(Request $request){
        //dump($this->session->get('userId')); die;
        // ini vars
       ...
       // dump($studentb);
        ...
        if((trim($studenta->getNombre()) == trim($studentb->getNombre()))
                ...
                //check if the user has tuition in the same UE 
                $tuicionUeA =$this->get('funciones')->getInscriptionToValidateTuicionUe($rudea, $this->currentyear);
                $tuicionUeB =$this->get('funciones')->getInscriptionToValidateTuicionUe($rudeb, $this->currentyear);
                if($tuicionUeA && $tuicionUeB){
                }
                else{
                    $swobservado = true;
                    $mensaje = 'Usted no tiene tuicion sobre ambas Unidades Educativas, el proceso debe ser realizado por la Dirección Distrital/Departamental';
                }

                //check if any the RUDEs corresponds to the current Year inscription 
                $insriptinoStudentA = $this->getCurrentInscriptionsByGestoinValida($rudea, $this->currentyear);
                $insriptinoStudentB = $this->getCurrentInscriptionsByGestoinValida($rudeb, $this->currentyear);
                
                if(!$insriptinoStudentA or !$insriptinoStudentB ){ 
                    $swobservado = true;
                    $mensaje = 'Alguno de los RUDES no corresponde al proceso de Inscripción de la gestion '. $this->currentyear .' ';
                }
        }else {
            $swobservado = true;
            $mensaje = 'Los datos del Estudiante no son iguales';
        }
