...
die;/*
	...
	public function actualizarEstadoMatricula($idInscripcion){
        try {
            ...
            $tipo = $this->getTipoNotaEsp($sie,$gestion,$nivel,$grado,'');
            $newNotaTipo = ($gestion==2021)?'9':'5';
            ...
            if($nivel == 11 or $nivel == 1 or $nivel == 403){
                ...
                foreach ($asignaturas as $a) {
                    // Notas Bimestrales
                    if(in_array($tipo, array('Bimestre','newTemplateDB'))  ){
                        $notaPromedio = $this->em->getRepository('SieAppWebBundle:EstudianteNota')->findOneBy(array('estudianteAsignatura'=>$a->getId(),'notaTipo'=>$newNotaTipo));
                        if($notaPromedio){
                            /**
