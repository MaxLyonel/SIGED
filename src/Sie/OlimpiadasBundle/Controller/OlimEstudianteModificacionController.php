...
/**
 * OlimEstudianteModificacion controller.
 *
 */
/**
 ...
 *
 *
 * OlimEstudianteModificacionController.php
 *
 * Email bugs/suggestions to <pckrlos@gmail.com>
 */

class OlimEstudianteModificacionController extends Controller{
  ...
  public function getGradosAction(Request $request){
    ...
    private function getListStudents($materiaId, $olimtutorid, $nivelId, $gradoId){
        ...
        $query = $em->getConnection()->prepare("
            (select distinct oei.id olimestudianteid, oei.telefono_estudiante, oei.correo_estudiante, e.codigo_rude, e.carnet_identidad, e.paterno, e.materno, e.nombre, e.fecha_nacimiento, odt.id dscpid, odt.discapacidad, ot.id tutorid, p.carnet carnet1, p.paterno paterno1, p.materno materno1, p.nombre nombre1, 'Grado regular' obs
        ...
        from olim_estudiante_inscripcion oei
        inner join estudiante_inscripcion ei on ei.id = oei.estudiante_inscripcion_id
        inner join olim_inscripcion_grupo_proyecto oigp on oei.id = oigp.olim_estudiante_inscripcion_id
        inner join olim_grupo_proyecto ogp on ogp.id = oigp.olim_grupo_proyecto_id
        inner join estudiante e on e.id = ei.estudiante_id
        inner join olim_discapacidad_tipo odt on odt.id = oei.discapacidad_tipo_id
        inner join olim_tutor ot on ot.id = oei.olim_tutor_id
        inner join persona p on p.id = ot.persona_id
        where ot.id = :olimTutor
        order by e.paterno, e.materno, e.nombre)*/
