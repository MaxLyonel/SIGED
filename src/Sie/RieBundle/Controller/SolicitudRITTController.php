...
class SolicitudRITTController extends Controller {
    ...
    public function vistaPreviaCertificadoAction(Request $request){
        ...
        if(count($resultados)>0)
            $response->setContent(file_get_contents($this->container->getParameter('urlreportweb') . 'rie_cert_certificadottec_vp_v3_ratificado_cvm.rptdesign&institucioneducativa_id='.$idRie.'&&__format=pdf&'));
        else
    ...
    public function guardaTramiteNacImprimeConcluidoAction(Request $request){
        ...
        $response->headers->set('Content-Disposition', sprintf('attachment; filename="%s"', $arch));
        if(count($resultados)>0)
            $response->setContent(file_get_contents($this->container->getParameter('urlreportweb') . 'rie_cert_certificadottec_v3_ratificado_cvm.rptdesign&documento_id='.$entityDocumento->getId().'&&__format=pdf&'));
        else
            $response->setContent(file_get_contents($this->container->getParameter('urlreportweb') . 'rie_cert_certificadottec_v3_afv.rptdesign&documento_id='.$entityDocumento->getId().'&&__format=pdf&'));
