...
// funcion para cambiar el madre, mostrar la opcion de busqueda
...
var m_buscarMadre = function(){
    ...
    if(m_carnet != "" && m_nombre != ""){

        $.ajax({
            type: 'get',
            url: Routing.generate('info_estudiante_rude_buscar_persona',{'carnet':m_carnet, 'complemento':m_complemento, 'paterno':m_paterno, 'materno': m_materno, 'nombre': m_nombre, 'fechaNacimiento': m_fechaNacimiento}),
            beforeSend: function(){
                $('#m_mensaje').empty();
...
//             $('#m_mensaje').empty();
//             m_cambiarFondoMensaje(3);
//             $('#m_mensaje').append('El apellido paterno del padre o tutor no coincide con el apellido paterno del estudiante');
//         }else{
//             $('#m_mensaje').empty();
//             m_cambiarFondoMensaje(0);
...
function saveFormMadre(recargar = true){
